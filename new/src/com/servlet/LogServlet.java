	...
	public void admin_log_list(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			...
			if(db.createConn()){
				...
				sql = "select * from admin_log where entry_time>='"+startTime+"' and entry_time<='"+endTime+"' and admin_name like '%"+adminName+"%' order by entry_time desc";
				}else{
					sql = "select * from admin_log where admin_name like '%"+adminName+"%' order by entry_time desc";
				}
				stmt= db.getStmtread();
				...
				while(rs.next()){
					...
					as.setAdminName(rs.getString("admin_name"));
	...
	public void edit(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			...
			if(db.createConn()){
				...
				System.out.println(token);
				request.getSession().setAttribute("token", String.valueOf(token));
				String sql = "select * from admin_log where id='"+id+"' order by entry_time desc";
		...
		try {
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 String sqli = "insert into admin_log(admin_name,contents,entry_time) values('"+admin.getAdminName()
							 +"','"+centents+"','"+date+"')";
					 stmt = conn.createStatement();
	...
	public void admin_log_delAll(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			...
			if(db.createConn()){
				String startTime = startTimeStr+" 00:00:00";
				String  endTime = endTimeStr+" 23:59:59";
				String sql = "select * from admin_log where entry_time>='"+startTime+"' and entry_time<='"+endTime+"' for update";
				Timestamp date1 = new Timestamp(new Date().getTime());
				 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
				stmt= db.getStmtread();
				rs =stmt.executeQuery(sql);
