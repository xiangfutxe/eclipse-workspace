...
public class NewsServlet extends HttpServlet {
			...
			if(db.createConn()){
				...
				if(!(startTimeStr.equals("")||endTimeStr.equals(""))){
					...
					String  endTime = endTimeStr+" 23:59:59";
							sql ="select * from news where entry_time>='"+startTime+"' and entry_time<='"+endTime+"' and sort like '%"+sort+"%' order by entry_time desc";
						
					}else{
							sql  ="select * from news where sort like '%"+sort+"%' order by entry_time desc";
						}
				stmt= db.getStmtread();
	...
	public void list(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			...
			if(db.createConn()){
				String sql  ="select * from news where state='1'  order by entry_time desc";
				stmt= db.getStmtread();
				rs =stmt.executeQuery(sql);
				while(rs.next()){
					News as = new News();
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 String sql = "insert into news(title,sort,contents,admin,state,entry_time,end_time) values('"+
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 String sql = "update news set title='"+title+"',contents='"+contents+"',sort='"+sort+"',end_time='"+date+"' where id='"+id+"'";
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 String sql = "update news set state='0',end_time='"+date+"' where id='"+id+"'";
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 String sql = "update news set state='1',end_time='"+date+"' where id='"+id+"'";
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
