...
User user_one = (User)request.getAttribute("user_one");
...
<div class="content">
	<div class="header">
        ...
        <p class="go-title-ch">
        	首次购物</p><p class="go-title-en">SHOPPING
        </p>
...
<div class="content">
    <div class="container">
     ...
     <form  action="OrderServlet.do?method=order_vip_save"  id="myform" name="myform" method="post">
		      <input type="hidden" name="userId" value='<%=user.getUserId()%>'/>
		       <input type="hidden" name="token" value='<%=token%>'/>
		       <input type="hidden" name="emoney" id="emoney" value='<%=user_one.getEmoney()%>'/>
		...
		$("#btn1").bind("click", function() {
			$.ajax({
				...
				success : function(data) {
					...
					else if (emoney > totalprice) $("#msg").text("您选购的产品总额与代理商标准不符合！").css({"color":"red","font-size":"12px"});
					else {$("#msg").text("");
