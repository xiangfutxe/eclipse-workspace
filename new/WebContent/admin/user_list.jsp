...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/UserServlet.do?method=adminList" id="searchForm"
						method="post">
						<table class="search-tab">
							<tr>
								<td width="800px">加盟级别：<select class="common-text" id="rankJoin"
									name="rankJoin">
										<option value="">--所有级别--</option>
										<option value="1" <%if(StringUtil.notNull(rankJoin).equals("1")){ %>selected<%} %>>银级代理商</option>
										<option value="2" <%if(StringUtil.notNull(rankJoin).equals("2")){ %>selected<%} %>>金级代理商</option>
										<option value="3" <%if(StringUtil.notNull(rankJoin).equals("3")){ %>selected<%} %>>钻级代理商</option>
								
								</select>  &nbsp;会员信息：<input class="common-text" name="userId" id="userId" size="15"
									type="text" value='<%=StringUtil.notNull(userId) %>'>
									&nbsp;客服编号：<input class="common-text"  name="refereeId" id="refereeId" type="text" size="15" value='<%=StringUtil.notNull(refereeId) %>'>
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							<th>用户编号</th><th>用户名</th><th>会员等级</th><th>客服编号</th><th>状态</th><th>注册时间</th><th>操作</th>
						</tr>
						  <%
 ...
 <td><a href='admin/UserServlet.do?method=admin_user_detail&id=<%=user.getUserId()%>'><%=StringUtil.notNull(user.getUserId())%></a></td>
 <td><%=StringUtil.notNull(user.getUserName())%></td>
 <td><%if(user.getRankJoin()==1){%>银级代理商<%}else if(user.getRankJoin()==2) {%>金级代理商
 <%}else if(user.getRankJoin()==3){%>钻级代理商
<% }else{%>-<%}%></td>

<td><%=StringUtil.notNull(user.getRefereeUserId())%></td>

 <td><%if(user.getState()==2){%>已激活<%}else if(user.getState()==1){%>锁定<%}else{%>下线<%}%></td>
 <td><%=StringUtil.parseToString(user.getEntryTime(), DateUtil.yyyyMMddHHmmss)%></td>
 <td><a href='admin/UserServlet.do?method=admin_user_detail&id=<%=user.getUserId()%>'>详情</a>
	 	...
	 	<%}}
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/UserServlet.do?method=adminList" method="post">
			...
			<form name="pageNoForm"  action="admin/UserServlet.do?method=adminList" method="post">
					...
					<a
						href="admin/UserServlet.do?method=adminList&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>
						&rankJoin=<%=StringUtil.notNull(rankJoin)%>">首页</a> | <a
						href="admin/UserServlet.do?method=adminList&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a 	href="admin/UserServlet.do?method=adminList&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">
                    下一页</a> | <a
						href="admin/UserServlet.do?method=adminList&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">尾页</a>
