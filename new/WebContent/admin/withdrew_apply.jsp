		 ...
		 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/WithDrewServlet.do?method=exportExcel");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
		...
		$('#search')
				.click(
						function() {
											...
											if (str == "") {
												$("#searchForm").attr("action", "admin/WithDrewServlet.do?method=admin_list");
												$('#searchForm').submit();
												return true;
											} else {
												alert(str);
					...
					<form  id="searchForm"
						...
						<table class="search-tab">
							<tr>
								...
								<input class="common-text" id="userId" name="userId" value="<%=StringUtil.notNull(userId) %>" type="text"/>
			...
			<div class="result-wrap">
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						<tr>
							<th>申请编号</th><th>提现金额</th><th>手续费</th><th>到账金额</th><th>账户信息</th><th>申请人</th><th>申请时间</th><th>备注</th><th>状态</th><th>操作</th>
						</tr>
					 <%
...
<td><%=StringUtil.notNull(ca.getComments())%></td>
 <td><%if(ca.getState()==1) {%>待审批<%}else if(ca.getState()==2){ %>
										...
										<a href="admin/WithDrewServlet.do?method=apply_comments_add&id=<%=ca.getId()%>">备注提示</a>
										<%if(ca.getState()==1) {%>&nbsp;|&nbsp;<a id="yes" href="admin/WithDrewServlet.do?method=apply_yes&id=<%=StringUtil.notNull(ca.getApplyId()) %>">审核通过 </a>&nbsp;|&nbsp;<a id='no' href="admin/WithDrewServlet.do?method=apply_no&id=<%=StringUtil.notNull(ca.getApplyId()) %>">审核不通过 </a><%}} %></td>
 </tr>
		<%}%>
							<tr>
							<td colspan="10" style="text-align:left;">提现合计：<%=StringUtil.decimalFormat(sum[0])%></td>
						 </tr>
							<%}
						}else{ %><tr>
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/WithDrewServlet.do?method=admin_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/WithDrewServlet.do?method=admin_list"
							method="post">
					...
					<a
						href="admin/WithDrewServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/WithDrewServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/WithDrewServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/WithDrewServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
