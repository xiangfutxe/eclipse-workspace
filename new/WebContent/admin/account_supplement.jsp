...
<%@page import="com.pojo.AccountSupplement"%>
...
double[] sum = (double[])request.getAttribute("sum");
	if (admin == null) {
		...
		if(rankstr[6][12].equals("1")||admin.getState().equals("1")){
...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/AccountSupplementServlet.do?method=list" id="searchForm" method="post" >
					...
					<option value="1" <%if(StringUtil.notNull(type).equals("1")){ %>selected<%} %>>报单券账户</option>
						...
						<option value="5" <%if(StringUtil.notNull(type).equals("6")){ %>selected<%} %>>促销币账户</option>
						</select>
							</td>
 ...
 <td><%if(as.getType()==1){ %>报单券账户
 <%} else if(as.getType()==4){ %>奖金券账户
<%} else if(as.getType()==5){ %>促销币账户
<%}else{ %>-<%} %></td>
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/AccountSupplementServlet.do?method=list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/AccountSupplementServlet.do?method=list"
							method="post">
					...
					<a
						href="admin/AccountSupplementServlet.do?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&type=<%=StringUtil.notNull(type)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/AccountSupplementServlet.do?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&type=<%=StringUtil.notNull(type)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/AccountSupplementServlet.do?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/AccountSupplementServlet.do?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&type=<%=StringUtil.notNull(type)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
