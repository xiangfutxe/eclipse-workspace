						...
						if(pager!=null){
							...
							%>
								<tr>
									...
									<a class="link-update"
										href="admin/AdminServlet.do?method=editRank&id=<%=ad.getId()%>">修改权限</a>&nbsp;|
										<%}if(rankstr[9][4].equals("1")||admin.getState().equals("1")){ %>&nbsp;<a
										...
										<a
										class="link-del"
										href="admin/AdminServlet.do?method=del&id=<%=ad.getId()%>" onclick="if(confirm('确定删除该管理员信息?')==false)return false;">删除</a>
										<%} %></td>
								</tr>
						<%}}
						}else{ %>
