		 ...
		 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/ChargeApplyServlet.do?method=exportExcel");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/ChargeApplyServlet.do?method=admin_chargeApply_list" id="searchForm"
						method="post">
			...
			<div class="result-wrap">
				...
				<div class="result-content">
						...
						if(pager!=null){
							...
							%>
								<tr>
									...
									<td><%=StringUtil.notNull(ca.getComments()) %>
									</td>
									...
									<td><a href="admin/ChargeApplyServlet.do?method=apply_comments_add&id=<%=ca.getId()%>">备注提示</a><%if(rankstr[6][1].equals("1")||admin.getState().equals("1")){if(ca.getState()==1) {%>
											&nbsp;|&nbsp;<a href="admin/ChargeApplyServlet.do?method=apply_yes&id=<%=ca.getApplyId()%>"
												onclick='cfm("确认审核通过？审核通过后会直接将金额转入会员账户！")'>审核通过</a>&nbsp;|&nbsp;<a
												href="admin/ChargeApplyServlet.do?method=apply_no&id=<%=ca.getApplyId() %>" onclick='cfm("确认审核不通过？")'>审核不通过</a>
										<%} }%>
									</td>
								</tr>
							<%}%>
							<tr>
							<td colspan="9" style="text-align:left;">充值合计：<%=StringUtil.decimalFormat(sum[0]) %></td>
						 </tr>
							<%}
						}else{ %><tr>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ChargeApplyServlet.do?method=admin_chargeApply_list" method="post">
			...
			<form name="pageNoForm"  action="admin/ChargeApplyServlet.do?method=admin_chargeApply_list" method="post">
			<input type="hidden" name="state" value="<%=StringUtil.notNull(state)%>"/>
							<input type="hidden" name="typeKey" value="<%=StringUtil.notNull(typeKey)%>"/>
							<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
							<input type="hidden" name="startTime" value="<%=StringUtil.notNull(startTime)%>"/>
					...
					<a
						href="admin/ChargeApplyServlet.do?method=admin_chargeApply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						...
						href="admin/ChargeApplyServlet.do?method=admin_chargeApply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ChargeApplyServlet.do?method=admin_chargeApply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/ChargeApplyServlet.do?method=admin_chargeApply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
