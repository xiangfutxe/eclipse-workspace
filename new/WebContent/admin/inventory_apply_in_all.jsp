		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					...
					<span class="crumb-step">&gt;</span><span class="crumb-name">库存管理</span>
					<span class="crumb-step">&gt;</span><span class="crumb-name">入库查询</span>
				</div>
			</div>
			...
			<div class="result-wrap">
						...
						if(pager!=null){
							...
							<tr>
								...
								<td><a class="link-update"
									href="admin/InventoryServlet.do?method=apply_in_all_detail&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									<%if(apply.getState()==1){%>
                              |&nbsp;  <a class="link-del"
											href="admin/InventoryServlet.do?method=apply_in_all_del&id=<%=StringUtil.notNull(apply.getApplyId())%>" onClick="delcfm()">删除</a>
									<%} %></td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_in_all&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_all&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
                    ...
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_all&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">尾页</a>
