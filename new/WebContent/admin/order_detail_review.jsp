			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						class="crumb-step">&gt;</span>订单管理<span class="crumb-step">&gt;</span><a href="">发货审核</a><span class="crumb-step">&gt;</span><span>订单详情</span>
				</div>
			</div>
...
<tr>
								...
								<tr>
									<th>快递费：</th>
									<td colspan='3'><%=StringUtil.decimalFormat(orders.getFee())%></td>
								</tr>
								<tr>
									<th>备注信息：</th>
					...
					<div class="result-content">
 					...
 					<tr <%if(t%2==1){%>class="even"<%}%>>
										...
										<td><%=StringUtil.decimalFormat(od.getProductPv()) %>
										</td>
					...
					<div class="result-title">
						...
						<br>
							...
							&nbsp;<%if(orders.getState()==3){ %><input type="button" class="btn" id="yesBtn" value="审核通过"/>
							&nbsp;<input type="button" class="btn" id="noBtn" value="审核不通过"/><%}%></p>
						</div>
					</div>
				</form>
			</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		$("#yesBtn").bind("click", function() {
			if (!confirm("确认要审核通过，请核对好出库清单？")) {
			window.event.returnValue = false;
		}else{
		 	$('#myform').attr("action", "admin/OrderServlet.do?method=admin_order_review_yes");
       						$('#myform').submit();
		}
		...
		$("#noBtn").bind("click", function() {
			...
			window.event.returnValue = false;
		 	...
		 	$('#myform').attr("action", "admin/OrderServlet.do?method=admin_order_review_no");
       						$('#myform').submit();
		}
