...
String endTime = (String)request.getAttribute("endTime");
	...
	String[][] rankstr = StringUtil.getRankStr(admin.getRank());
		if(rankstr[4][11].equals("1")||admin.getState().equals("1")){
%>
		...
		<div class="main-wrap">
			...
			<div class="result-wrap">
				...
				<div class="list-page">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_in&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
                    ...
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>">尾页</a>
