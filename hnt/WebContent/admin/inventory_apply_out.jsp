...
String type = StringUtil.notNull((String)request.getAttribute("type"));
	...
	String[][] rankstr = StringUtil.getRankStr(admin.getRank());
		if(rankstr[4][5].equals("1")||admin.getState().equals("1")){
%>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/InventoryServlet?method=apply_out" id="searchForm" method='post'>
						<table class="search-tab">
							...
							%>
								...
								</select>&nbsp;出库类型：<select name="type" id="type" class="common-text" >
                            <option value="">--请选择出库类型--</option>
                            <option value="1" <%if(type.equals("1")){ %>selected<%} %>>--加工出库--</option>
                             <option value="2" <%if(type.equals("2")){ %>selected<%} %>>--损耗出库--</option>
                              <option value="3" <%if(type.equals("3")){ %>selected<%} %>>--外销出库--</option>
                             <option value="4" <%if(type.equals("4")){ %>selected<%} %>>--体验出库--</option>
                              <option value="5" <%if(type.equals("5")){ %>selected<%} %>>--赠送出库--</option>
                               <option value="6" <%if(type.equals("6")){ %>selected<%} %>>--研发出库--</option>
                               <option value="7" <%if(type.equals("7")){ %>selected<%} %>>--退货出库--</option>
                               <option value="8" <%if(type.equals("8")){ %>selected<%} %>>--盘亏出库--</option>
                            </select></td></tr><tr>
								<td>申请时间:<input class="common-text" name="startTime" id="startTime" size="15"
			...
			<div class="result-wrap">
				<div class="result-title">
					<div class="result-list">
						<a href="admin/InventoryServlet?method=apply_out_add"><i
							class="icon-font">&#xe026;</i>新增出库</a>
					</div>
				</div>
					...
					<table class="result-tab" width="100%">
						...
						if(pager!=null){
							...
							<tr>
								...
								<td>
								<%if(apply.getType()==1){ %>加工出库
								<%}else if(apply.getType()==2){ %>损耗出库
								<%}else if(apply.getType()==3){ %>外销出库
								<%}else if(apply.getType()==4){ %>体验出库
								<%}else if(apply.getType()==5){ %>赠送出库
								<%}else if(apply.getType()==6){ %>研发出库
								<%}else if(apply.getType()==7){ %>退货出库
								<%}else if(apply.getType()==8){ %>盘亏出库
								<%} %>
								</td>
								...
								<td><a class="link-update"
									href="admin/InventoryServlet?method=apply_out_detail&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									<%if(apply.getState()==1){%>
                              |&nbsp;  <a class="link-del"
											href="admin/InventoryServlet?method=apply_out_del&id=<%=StringUtil.notNull(apply.getApplyId())%>" onClick="delcfm()">删除</a>
									<%} %></td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/InventoryServlet?method=apply_out"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/InventoryServlet?method=apply_out"
							method="post">
					...
					<a
						href="admin/InventoryServlet?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>&type=<%=StringUtil.notNull(type)%>">首页</a> | <a
						href="admin/InventoryServlet?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>&type=<%=StringUtil.notNull(type)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/InventoryServlet?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/InventoryServlet?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&inventoryId=<%=StringUtil.notNull(inventoryId)%>&type=<%=StringUtil.notNull(type)%>">尾页</a>
