...
$(function() {
	...
	$("#summary").blur(function() {
		if(getByteLen($("#summary").val())>50) $("#summaryTag").text("摘要字数不能大于25！").css({"color":"red","font-size":"12px"});
	...
	$("#userId").blur(function() {
		$.ajax({
				type : "post",
				url : "admin/UserServlet?method=userAjax_admin",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
					userId : $("#userId").val()
	...
	if (!confirm("确认提交补扣，请认真核对补扣信息？")) {
		...
		if($("#payType").val()==""){
				$("#payTypeTag").text("补扣类型不能为空！").css({"color":"red","font-size":"12px"});
				errorstr = errorstr+"1";
			}
		...
		if(errorstr!=""){
			...
			$('#myform').attr("action", "admin/AccountSupplementServlet?method=save");
	       	$('#myform').submit();
       		 return true;
