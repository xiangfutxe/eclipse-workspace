   ...
   <div class="main-wrap">
        ...
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/ParamServlet?method=list" method="post">
           ...
           <form name="delform" id="delform"  method="post">
                <div class="result-title">
                    <div class="result-list">
                        <a href="admin/ParamServlet?method=addParam"><i class="icon-font">&#xe026;</i>新增参数</a>
                    </div>
                </div>
                    ...
                    <table class="result-tab" width="100%">
						...
						if(pager!=null){
						 ...
						 <td> <%=StringUtil.decimalFormat(ad.getAmount_10()) %></td>
						  ...
						  <td>
                                <a class="link-update" href="admin/ParamServlet?method=editParam&id=<%=ad.getId()%>">修改</a>&nbsp;|&nbsp;
                                <a class="link-update" href="admin/ParamServlet?method=removeParam&id=<%=ad.getId()%>" onClick="delcfm()">删除</a>
                            </td>
						 </tr>
						<%}}
						}else{ %>
						 <tr>
						 <td colspan="13">添加参数，请点击<a href="admin/ParamServlet?method=add">添加参数。</a></td>
						  </tr>
						 <%} %>
						</table>
			...
			<%if(pager!=null){ %>
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ParamServlet?method=list" method="post">
			...
			<form name="pageNoForm"  action="admin/ParamServlet?method=list" method="post">
					<input type="hidden" name="nameKey" value="<%=StringUtil.notNull(nameKey)%>"/>
					...
					<a
						href="admin/ParamServlet?method=list&nameKey=<%=StringUtil.notNull(nameKey)%>&pageSize=<%=pager.getPageSize()%>&pageNoStr=1">首页</a> | <a
						href="admin/ParamServlet?method=list&nameKey=<%=StringUtil.notNull(nameKey)%>&pageSize=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ParamServlet?method=list&nameKey=<%=StringUtil.notNull(nameKey)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a> | <a
						href="admin/ParamServlet?method=list&nameKey=<%=StringUtil.notNull(nameKey)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
