...
<script language="JavaScript">
    ...
    $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
		...
		$('#search')
			 ...
			 $("#exportExcel_detail_all").click(function(){
	  	  if (!confirm("确认提交导出所选订单明细？")) {
				return false;
			}
			else{
	       		$("#delform").attr("action", "admin/OrderServlet?method=exportExcel_orders_detail_all");
				$("#delform").submit();
	       		 return true;
			}
		 });
...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet?method=admin_order_delivery_wait" id="searchForm" method="post">
					...
					<option value="1" <%if(StringUtil.notNull(orderType).equals("1")){ %>selected<%} %>>报单购物</option>
						...
						<option value="6" <%if(StringUtil.notNull(orderType).equals("6")){ %>selected<%} %>>消费购物</option>
						</select>
							</td>
                ...
                <div class="result-title">
                    <div class="result-list">
                      ...
                      <%if(rankstr[5][7].equals("1")||admin.getState().equals("1")){%>
                       <button class="btn"  id="exportExcel_detail_all" name="exportExcel_detail_all">明细导出</button>
                      <%} %> 
                    </div>
                </div>
 ...
 <td><%if(order.getOrderType()==1){ %>报单购物<%}else if(order.getOrderType()==2){ %>零售购物<%}else if(order.getOrderType()==3){ %>复消购物<%}else if(order.getOrderType()==4){%>购物券购物<%}else if(order.getOrderType()==5){%>升级购物<%}else if(order.getOrderType()==6){%>消费购物
  <%}else{%>-<%} %></td>
...
<td><%if(order.getState()==1){%>待确认<%}else if(order.getState()==2){%>待发货<%}else if(order.getState()==3){%>出库中<%}else if(order.getState()==4){%>已发货<%}else if(order.getState()==5){%>已收货<%}else if(order.getState()==0){%>已退订<%} %></td>
  <td><a href="admin/OrderServlet?method=admin_order_detail_delivery_wait&id=<%=order.getOrderId()%>">详情</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet?method=admin_order_delivery_wait"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet?method=admin_order_delivery_wait"
							method="post">
					...
					<a
						href="admin/OrderServlet?method=admin_order_delivery_wait&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet?method=admin_order_delivery_wait&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet?method=admin_order_delivery_wait&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet?method=admin_order_delivery_wait&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
