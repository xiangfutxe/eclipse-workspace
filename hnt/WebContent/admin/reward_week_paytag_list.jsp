...
String userId = (String) request.getAttribute("userId");
	if (admin == null) {
		...
		if(rankstr[7][9].equals("1")||admin.getState().equals("1")){
%>
		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				...
				<div class="search-content">
					<form action="admin/RewardServlet?method=admin_reward_week_paytag_list" id="searchForm"
						method="post">
...
<td><%if(w.getState()==1){%>待发工资<%}else if(w.getState()==2){ %>已发工资<%}else if(w.getState()==3){%>拒发工资<%}else{%>结算中<%} %></td>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/RewardServlet?method=admin_reward_week_paytag_list" method="post">
...
<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
			...
			<form name="pageNoForm"  action="admin/RewardServlet?method=admin_reward_week_paytag_list" method="post">
...
<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
					...
					<a
						href="admin/RewardServlet?method=admin_reward_week_paytag_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&weekTag=<%=weekTag%>">首页</a> | <a
						href="admin/RewardServlet?method=admin_reward_week_paytag_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&weekTag=<%=weekTag%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/RewardServlet?method=admin_reward_week_paytag_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>&weekTag=<%=weekTag%>">
                    下一页</a> | <a
						href="admin/RewardServlet?method=admin_reward_week_paytag_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&weekTag=<%=weekTag%>">尾页</a>
		...
		$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/RewardServlet?method=exportExcel_payTag");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
