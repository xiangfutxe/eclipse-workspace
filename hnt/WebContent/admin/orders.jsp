	 ...
	 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/OrderServlet?method=exportExcel_orders");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
		 
		  $("#exportExcel_detail").click(function(){
	  	  if (!confirm("确认提交导出订单产品汇总？")) {
				return false;
			}
			else{
	       		$("#searchForm").attr("action", "admin/OrderServlet?method=exportExcel_orders_detail");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet?method=admin_orders" id="searchForm"
						method="post">
					...
					<option value="1" <%if(StringUtil.notNull(orderType).equals("1")){ %>selected<%} %>>报单购物</option>
						...
						<option value="10" <%if(StringUtil.notNull(state).equals("10")){ %>selected<%} %>>非退订</option>
                    	...
                    	<%if(rankstr[5][6].equals("1")||admin.getState().equals("1")){%>
									...
									name="exportExcel" value="导出订单" type="button">
									&nbsp;<input class="btn btn-info" id="exportExcel_detail"
									name="exportExcel_detail" value="导出产品汇总" type="button">
									<%} %><span
                ...
                <div class="result-title">
                    <div class="result-list">
                      ...
                      <%if(rankstr[5][7].equals("1")||admin.getState().equals("1")){%>
                       <button class="btn"  id="exportExcel_detail_all" name="exportExcel_detail">明细导出</button>
                      <%} %> 
                    </div>
                </div>
				<div class="result-content">
					<table class="result-tab" width="97%">
						...
						<th class="tc" width="20px"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							...
							<th>订单编号</th><th>订单类型</th><th>总金额</th><th>总PV</th><th>会员信息</th><th>收件人</th><th>联系电话</th><th>服务店</th><th>订单日期</th><th>当前状态</th><th>操作</th>
 ...
 <%}else if(order.getState()==0){%>已退单<%}%></td>
  <td><a href="admin/OrderServlet?method=admin_order_detail&id=<%=order.getOrderId()%>">详情</a></td>
	 	...
	 	<tr><td colspan="12">订单金额合计：<%=StringUtil.decimalFormat(sum[0]) %>元，pv合计：<%=StringUtil.decimalFormat(sum[1]) %></td></tr>
