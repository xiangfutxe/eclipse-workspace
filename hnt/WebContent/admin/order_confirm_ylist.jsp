...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet.do?method=admin_order_confirm_ylist" id="searchForm" method="post">
 ...
 <%}else if(order.getOrderType()==4){%>升级购物<%}else if(order.getOrderType()==6){%>创业购物<%}else{%>-<%} %></td>
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet.do?method=admin_order_confirm_ylist"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet.do?method=admin_order_confirm_ylist"
							method="post">
					...
					<a
						href="admin/OrderServlet.do?method=admin_order_confirm_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_confirm_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet.do?method=admin_order_confirm_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_confirm_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
