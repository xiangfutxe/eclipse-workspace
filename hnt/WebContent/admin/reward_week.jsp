         ...
         <form id="myform" name="myform"  method="post">
		  <input type="hidden" name="token" id="token" value="<%=StringUtil.notNull(token)%>"/> 
		   ...
		   <input type="hidden" name="startTime" id="startTime" value="<%=startTime%>"/> 
            ...
            <% if(tag!=1){%> <tr><td> <input id="btn1" class="btn" value="确认周期" type="button" <%if(tag!=2){ %>disabled="disabled"<%} %>/></td></tr>
            <%} %>
            <% if(tag==1){
 ...
 <script type="text/javascript">
	$(function() {
	  	  ...
	  	  if (!confirm("确认提交结算周期进行结算！")) {
				...
				$("#myform").attr("action", "admin/RewardServlet?method=admin_reward_week_0");
	       		 $("#myform").submit();
	       		 return  true;
			}
		 });
		  ...
		  $("#btn2").click(function(){
	  	  if (!confirm("确认提交开始进行结算，结算可能需要花费一点时间，确认后请耐心等待！")) {
				...
				$("#myform").attr("action", "admin/RewardServlet?method=admin_reward_week_1");
	       		 $("#myform").submit();
	       		 return true;
			}
		 });
		  ...
		  $("#btn4").click(function(){
			...
			$('#myform').attr("action", "admin/RewardServlet?method=admin_reward_week_reset");
	       		 $('#myform').submit();
	       		return true;
			}
		 });
		  ...
		  $("#btn3").click(function(){
			...
			$('#myform').attr("action", "admin/RewardServlet?method=admin_reward_week_confirm");
	       		 $('#myform').submit();
	       		 return true;
			}
		 });
