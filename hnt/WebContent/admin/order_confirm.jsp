...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet?method=admin_order_confirm" id="searchForm" method="post">
 ...
 <td><a href="admin/OrderServlet?method=admin_order_detail_confirm&id=<%=order.getOrderId()%>">详情</a>&nbsp;|&nbsp;<a href="admin/OrderServlet?method=admin_order_adr_edit&id=<%=order.getOrderId()%>">修改收货地址</a>
 <%if(order.getOrderType()==2||order.getOrderType()==3||order.getOrderType()==6){ %>&nbsp;|&nbsp;<a href="admin/OrderServlet?method=admin_order_back&id=<%=order.getOrderId()%>" onclick='delcfm()'>退单</a><%} %></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet?method=admin_order_confirm"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet?method=admin_order_confirm"
							method="post">
					...
					<a
						href="admin/OrderServlet?method=admin_order_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet?method=admin_order_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet?method=admin_order_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet?method=admin_order_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
