...
<html>
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">隐藏产品</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/ProductServlet?method=list_hide" method="post">
 ...
 Pager pager = (Pager)request.getAttribute("pager");
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ProductServlet?method=list_hide" method="post">
...
<input type="hidden" name="type" value="<%=StringUtil.notNull(type)%>"/>
			...
			<form name="pageNoForm"  action="admin/ProductServlet?method=list_hide" method="post">
					<input type="hidden" name="productName" value="<%=StringUtil.notNull(productName)%>"/>
					<input type="hidden" name="typeName" value="<%=StringUtil.notNull(typeName)%>"/>
					<input type="hidden" name="type" value="<%=StringUtil.notNull(type)%>"/>
					...
					<a
						href="admin/ProductServlet?method=list_hide&productName=<%=StringUtil.notNull(productName)%>&typeName=<%=StringUtil.notNull(typeName)%>&type=<%=StringUtil.notNull(type)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a> | <a
						href="admin/ProductServlet?method=list_hide&productName=<%=StringUtil.notNull(productName)%>&typeName=<%=StringUtil.notNull(typeName)%>&type=<%=StringUtil.notNull(type)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ProductServlet?method=list_hide&productName=<%=StringUtil.notNull(productName)%>&typeName=<%=StringUtil.notNull(typeName)%>&type=<%=StringUtil.notNull(type)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a> | <a
						href="admin/ProductServlet?method=list_hide&productName=<%=StringUtil.notNull(productName)%>&typeName=<%=StringUtil.notNull(typeName)%>&type=<%=StringUtil.notNull(type)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
