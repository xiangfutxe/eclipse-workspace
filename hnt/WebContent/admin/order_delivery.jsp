...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet?method=admin_order_delivery" id="searchForm" method="post">
					...
					<option value="1" <%if(StringUtil.notNull(orderType).equals("1")){ %>selected<%} %>>报单购物</option>
						...
						<option value="6" <%if(StringUtil.notNull(orderType).equals("6")){ %>selected<%} %>>创业购物</option>
								...
								<th width="70">会员编号</th><td><input class="common-text" name="userId" id="userId" size="15"
			...
			<div class="result-wrap">
				 <form name="delform" id="delform"  method="post" action="admin/OrderServlet?method=admin_order_detail_delivery_summary">
				...
				<div class="result-content">
					<table class="result-tab" width="99%">
						...
						<th class="tc" width="20px"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							...
							<th>订单编号</th><th>配货单号</th><th>订单类型</th><th>总金额</th><th>会员信息</th><th>收件人</th><th>联系电话</th><th>发货地点</th><th>订单日期</th><th>当前状态</th><th>操作</th>
 ...
 <tr <%if(t%2==1){%>class="even"<%}%>>
  ...
  <td><%=StringUtil.notNull(order.getToOrderId()) %></td>
 <td><%=StringUtil.notNull(order.getOrderId()) %></td>
 <td><%if(order.getOrderType()==1){ %>报单购物<%}else if(order.getOrderType()==2){ %>零售购物<%}else if(order.getOrderType()==3){ %>复消购物<%}else if(order.getOrderType()==4){%>购物券购物<%}else if(order.getOrderType()==5){%>升级购物<%}else if(order.getOrderType()==7){%>代金券购物
  <%}else if(order.getOrderType()==8){%>店铺进货<option value="9" <%if(StringUtil.notNull(orderType).equals("9")){ %>selected<%} %>>层碰购物</option>

 <%}else if(order.getOrderType()==9){%>层碰购物<%}else{%>-<%} %></td>
 ...
 <td><%=StringUtil.notNull(order.getReceiver()) %></td>
  <td><%=StringUtil.notNull(order.getPhone()) %></td>
   <td><%=StringUtil.notNull(order.getInventory()) %></td>
 <td><%=StringUtil.parseToString(order.getOrderTime(), DateUtil.yyyyMMddHHmmss)%></td>
 <td><%if(order.getState()==1){%>待发货<%}else if(order.getState()==2){%>出库中<%}else if(order.getState()==3){%>已发货<%}else if(order.getState()==4){%>已收货<%}else if(order.getState()==0){%>已退订<%} %></td>
  <td><a href="admin/OrderServlet?method=admin_order_detail_delivery&id=<%=order.getOrderId()%>">详情</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet?method=admin_order_delivery"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet?method=admin_order_delivery"
							method="post">
					...
					<a
						href="admin/OrderServlet?method=admin_order_delivery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet?method=admin_order_delivery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet?method=admin_order_delivery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet?method=admin_order_delivery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
