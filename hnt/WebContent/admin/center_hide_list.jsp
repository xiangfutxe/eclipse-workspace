...
String endTime = (String)request.getAttribute("endTime");
	if (admin == null) {
		...
		if(rankstr[1][17].equals("1")||admin.getState().equals("1")){
%>
	...
	$(function() {
		...
		$("#center_hide").click(function() {
			...
			var i = 0;
				...
				$('#myform').attr("action", "admin/CenterServlet?method=admin_center_hide_reset");
				$('#myform').submit();
					return true;
				}
			}
		...
		$("#center_hide_list").click(function() {
				$('#myform').attr("action", "admin/CenterServlet?method=list");
				$('#myform').submit();
					return true;
		});
		
		$("#center_add").click(function() {
				$('#myform').attr("action", "admin/CenterServlet?method=admin_center_add");
				$('#myform').submit();
					return true;
		});
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						<span
						class="crumb-step">&gt;</span><span class="crumb-name">服务中心(隐藏)</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/CenterServlet?method=list_hide" id="searchForm"
						method="post">
			 ...
			 <div class="result-title">
                 ...
                 <div class="result-title">
                    <div class="result-list">
                         ...
                         <button id="center_hide" class="btn"><i class="icon-font">&#xe021;</i>显示服务店</button>
                         <button id="center_hide_list" class="btn"><i class="icon-font">&#xe022;</i>服务店列表</button>
				...
				<div class="result-content">
					<table class="result-tab" width="97%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							<th>中心编号</th><th>中心类型</th><th>中心名称</th><th>用户信息</th><th>联系电话</th><th>经营区域</th><th>经营地址</th><th>工作室形式</th><th>面积</th><th>营业执照</th><th>当前状态</th><th>操作</th>
						</tr>
						 <%
...
<td><%=StringUtil.notNull(center.getPhone()) %></td>
 ...
 <td><a  href="admin/CenterServlet?method=admin_hide_detail_list&userId=<%=StringUtil.notNull(center.getUserId()) %>">详情</a>
 </td>
	...
	<%}}
			  ...
			  <%if(pager!=null){ %>
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/CenterServlet?method=list_hide" method="post">
			...
			<form name="pageNoForm"  action="admin/CenterServlet?method=list_hide" method="post">
			<input type="hidden" name="state" value="<%=StringUtil.notNull(state)%>"/>
							<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
					...
					<a
						href="admin/CenterServlet?method=list_hide&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&userId=<%=StringUtil.notNull(userId)%>&state=<%=StringUtil.notNull(state)%>">首页</a> | <a
						href="admin/CenterServlet?method=list_hide&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/CenterServlet?method=list_hide&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&userId=<%=StringUtil.notNull(userId)%>&state=<%=StringUtil.notNull(state)%>">
                    下一页</a> | <a
						href="admin/CenterServlet?method=list_hide&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
