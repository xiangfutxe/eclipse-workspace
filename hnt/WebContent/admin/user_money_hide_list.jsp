...
<script language="JavaScript">
	...
	$(function() {
		...
		$("#user_list").click(function() {
				$('#myform').attr("action", "admin/UserServlet?method=admin_user_money_list");
				$('#myform').submit();
					return true;
		});
		 ...
		 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/UserServlet?method=exportExcel_user_money_hide_list");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						<span
						class="crumb-step">&gt;</span><span class="crumb-name">隐藏列表</span>
						<span
						class="crumb-step">&gt;</span><span class="crumb-name">会员账户</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/UserServlet?method=admin_user_money_hide_list" id="searchForm"
						method="post">
						<table class="search-tab">
							<tr>
								<td width="800px">&nbsp;会员信息：<input class="common-text" name="userId" id="userId" size="15"
									...
									<%if(rankstr[1][26].equals("1")||admin.getState().equals("1")){ %>
			...
			<form  id="myform" method="post">
				 <div class="result-title">
                    <div class="result-list">
                       ...
                       <button id="user_list" class="btn"><i class="icon-font">&#xe022;</i>会员账户</button>
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						<tr>
							<th>用户编号</th><th>用户名</th><th>报单券</th><th>购物券</th><th>复消券</th><th>奖金券</th><th>电子券</th><th>电子券有效期</th><th>总收入</th>
						</tr>
						  <%
...
<td><%=StringUtil.decimalFormat(user.getEmoney())%></td>
								<td><%=StringUtil.decimalFormat(user.getSmoney())%></td>
								<td><%=StringUtil.decimalFormat(user.getDmoney())%></td>
								<td><%=StringUtil.decimalFormat(user.getRmoney())%></td>
									<td><%=StringUtil.decimalFormat(user.getPmoney())%></td>
<td><%=StringUtil.parseToString(user.getValidtyTime(), DateUtil.yyyyMMddHHmmss)%></td>
									<td><%=StringUtil.decimalFormat(user.getTotalIncome())%></td>
	 	...
	 	<%}}
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/UserServlet?method=admin_user_money_hide_list" method="post">
			...
			<form name="pageNoForm"  action="admin/UserServlet?method=admin_user_money_hide_list" method="post">
					...
					<a
						href="admin/UserServlet?method=admin_user_money_hide_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/UserServlet?method=admin_user_money_hide_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>
                 ...
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
                    ...
                    下一页</a> | <a
						href="admin/UserServlet?method=admin_user_money_hide_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
