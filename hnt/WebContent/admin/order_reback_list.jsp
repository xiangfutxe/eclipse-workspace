...
<body>
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">退货管理</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet.do?method=admin_order_reback" id="searchForm" method="post">
					...
					<option value="1" <%if(StringUtil.notNull(orderType).equals("1")){ %>selected<%} %>>报单购物</option>
						...
						<option value="7" <%if(StringUtil.notNull(orderType).equals("7")){ %>selected<%} %>>代金券购物</option>
						<option value="8" <%if(StringUtil.notNull(orderType).equals("8")){ %>selected<%} %>>店铺进货</option>
						<option value="9" <%if(StringUtil.notNull(orderType).equals("9")){ %>selected<%} %>>层碰购物</option>
						</select>
							</td>
				...
				<div class="result-content">
					<table class="result-tab" width="99%">
						<tr>
							...
							<th>订单编号</th><th>订单类型</th><th>总金额</th><th>会员信息</th><th>收件人</th><th>联系电话</th><th>发货地点</th><th>订单日期</th><th>当前状态</th><th>操作</th>
 ...
 <td><%if(order.getOrderType()==1){ %>报单购物<%}else if(order.getOrderType()==2){ %>零售购物<%}else if(order.getOrderType()==3){ %>复消购物<%}else if(order.getOrderType()==4){%>购物券购物<%}else if(order.getOrderType()==5){%>升级购物<%}else if(order.getOrderType()==9){%>层碰购物<%}else{%>-<%} %></td>
 ...
 <td><a href="admin/OrderServlet.do?method=admin_order_detail_reback&id=<%=order.getOrderId()%>">详情</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet.do?method=admin_order_review_ylist"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet.do?method=admin_order_review_ylist"
							method="post">
					...
					<a
						href="admin/OrderServlet.do?method=admin_order_review_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_review_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet.do?method=admin_order_review_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_review_ylist&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
