...
<html>
	...
	$("#abtn").click(function(){
		if (!confirm("确认提交申请？")) {
        	...
        	if($("#inventoryId").val()=="") error=error+"未选择仓库;<br>";
        	        	if($("#type").val()=="") error=error+"请选择出库类型;<br>";
 			...
 			if(error==""){
         		$('#saveform').attr("action", "admin/InventoryServlet?method=apply_in_save");
       		 	$('#saveform').submit();
       		 	return true;
       		 }else{
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="admin/index.jsp">首页</a><span class="crumb-step">&gt;</span><span class="crumb-name">入库管理</span><span class="crumb-step">&gt;</span><span class="crumb-name">新增入库</span></div>
        </div>
       
        <div class="result-wrap">
           <form action="admin/InventoryServlet?method=apply_in_save" method="post" id="saveform" name="saveform">
 ...
 Collection coll = (Collection)request.getAttribute("coll");
						 ...
						 <td> <%=StringUtil.notNull(p.getProductId())%>
						   <input type="hidden" name="pids" value="<%=StringUtil.notNull(p.getProductId())%>"> </td>
					...
					<td>
						  <input type="text" size="10" name="prices" class="common-text"> </td>
						  <td ><input type="text" size="10" name="nums" class="common-text"> <input type="hidden"  name="ids" value='<%=p.getId()%>'> 
						 </tr>
						<%}
						}else{ %><tr>
