...
String userId = (String) request.getAttribute("userId");
	if (admin == null) {
		...
		if(rankstr[7][21].equals("1")||admin.getState().equals("1")){
%>
		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				...
				<div class="search-content">
					<form action="admin/DRewardServlet?method=admin_reward_day_list" id="searchForm"
						method="post">
			...
			<form  id="myform" method="post" action="admin/DRewardServlet?method=admin_reward_day_payoff">
			<input type="hidden" name="startTime" value="<%=startTime%>"/>
			<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
			<%if(admin.getAdminName().equals("admin")||rankstr[7][24].equals("1")){ %>
...
<td><%if(w.getState()==1){%>待发工资<%}else if(w.getState()==2){ %>已发工资<%}else if(w.getState()==3){%>拒发工资<%}else{%>结算中<%} %></td>
						...
						}else{ %><tr><td colspan="15">暂无记录！</td></tr><%} %>
					</table>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/DRewardServlet?method=admin_reward_day_list" method="post">
...
<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
			...
			<form name="pageNoForm"  action="admin/DRewardServlet?method=admin_reward_day_list" method="post">
...
<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
					...
					<a
						href="admin/DRewardServlet?method=admin_reward_day_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&startTime=<%=startTime%>">首页</a> | <a
						href="admin/DRewardServlet?method=admin_reward_day_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&startTime=<%=startTime%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/DRewardServlet?method=admin_reward_day_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>&startTime=<%=startTime%>">
                    下一页</a> | <a
						href="admin/DRewardServlet?method=admin_reward_day_list&userId=<%=StringUtil.notNull(userId)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&startTime=<%=startTime%>">尾页</a>
		...
		$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/DRewardServlet?method=exportExcel");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
