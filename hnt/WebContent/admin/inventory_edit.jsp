  ...
  <div class="container clearfix">
   <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="admin/index.jsp">首页</a><span class="crumb-step">&gt;</span><a class="crumb-name">仓库管理</a><span class="crumb-step">&gt;</span><span>修改仓库</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                <form action="admin/InventoryServlet?method=update" method="post" id="myform" name="myform">
                 <input type="hidden" name="token" id="token" value="<%=StringUtil.notNull(token)%>"/> 
               <input type="hidden" name="id" id="id" value="<%=inventory.getId()%>"/> 
                    ...
                    <table class="insert-tab" width="100%">
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <input class="common-text required" id="inventoryName" name="inventoryName" value="<%=inventory.getInventoryName() %>" type="text">&nbsp;<span id="nameTag"></span>
                                </td>
                            </tr>
                             <tr>
                                <th><i class="require-red">*</i>仓库所在地：</th>
                                <td>
                                    <input class="common-text required" id="address" name="address" value="<%=inventory.getAddress() %>" type="text">&nbsp;<span id="addressTag"></span>
                                </td>
                            </tr>
                             <tr>
                                <th><i class="require-red">*</i>仓库负责人：</th>
                                <td>
                                    <input class="common-text required" id="linkman" name="linkman" value="<%=inventory.getLinkman() %>" type="text">&nbsp;<span id="linkmanTag"></span>
                                </td>
                            </tr>
                             <tr>
                                <th><i class="require-red">*</i>联系电话：</th>
                                <td>
                                    <input class="common-text required" id="phone" name="phone" value="<%=inventory.getPhone() %>" type="text">&nbsp;<span id="phoneTag"></span>
                                </td>
                            </tr>
                            <tr>
                                <th></th>
...
$(function() {
 var reg =/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/; //电话号码与手机号码同时验证
	...
	$("#inventoryName").blur(function() {
		$.ajax({
				type : "post",
				url : "admin/InventoryServlet?method=isExitUpdate",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
