	...
	$(function() {
  	  ...
  	  if (!confirm("确认要审核通过？")) {
				return false;
						...
						$("#myform").attr("action",'admin/InventoryServlet?method=apply_in_review_yes');
					$("#myform").submit();
		       		 return true;
		 }   
	   });
    ...
    $("#rnbtn").click(function(){
  	  if (!confirm("确认要审核不通过？")) {
				return false;
						...
						$("#myform").attr("action",'admin/InventoryServlet?method=apply_in_review_no');
					$("#myform").submit();
		       		 return true;
		 }   
	   });
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						class="crumb-step">&gt;</span><span class="admin/InventoryServlet?method=apply_in_review">入库审核</span><span
						class="crumb-step">&gt;</span><span class="crumb-name">入库产订单详情</span>
				</div>
			</div>
					...
					<table class="result-tab" width="100%">
						<tr>
							...
							<tr>
								...
								<td>
								<%if(ap.getType()==1){ %>采购入库
								<%}else if(ap.getType()==2){ %>加工入库
								<%}else if(ap.getType()==3){ %>退货入库
								<%}else if(ap.getType()==4){ %>盘盈入库
								<%} %>
								</td>
								<td>当前状态</td>
