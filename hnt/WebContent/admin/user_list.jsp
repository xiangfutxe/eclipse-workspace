		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/UserServlet?method=admin_list" id="searchForm"
						method="post">
						<table class="search-tab">
							<tr>
								<td width="800px">会员等级：<select class="common-text" id="rankJoin"
									name="rankJoin">
										<option value="">--所有级别--</option>
										<option value="1" <%if(StringUtil.notNull(rankJoin).equals("1")){ %>selected<%} %>>银卡会员</option>
										<option value="2" <%if(StringUtil.notNull(rankJoin).equals("2")){ %>selected<%} %>>金卡会员</option>
										<option value="3" <%if(StringUtil.notNull(rankJoin).equals("3")){ %>selected<%} %>>白金会员</option>
										<option value="4" <%if(StringUtil.notNull(rankJoin).equals("4")){ %>selected<%} %>>VIP会员</option>
										<option value="5" <%if(StringUtil.notNull(rankJoin).equals("5")){ %>selected<%} %>>至尊会员</option>
								</select> &nbsp;会员信息：<input class="common-text" name="userId" id="userId" size="15"
							...
							&nbsp;注册日期：
								...
								<input class="common-text" name="endTime" id="endTime" size="15"
									...
									<td width="800px">&nbsp;服务商：<input class="common-text"  name="refereeId" id="refereeId" type="text" size="15" value='<%=StringUtil.notNull(refereeId) %>'>
									&nbsp;销售商：<input class="common-text"  name="belongId" id="belongId" type="text" size="15" value='<%=StringUtil.notNull(belongId) %>'>
									&nbsp;报单人：<input class="common-text"  name="declarationId" id="declarationId" type="text" size="15" value='<%=StringUtil.notNull(declarationId) %>'>&nbsp;
			...
			<form  id="myform" method="post">
				 <div class="result-title">
                    ...
                    <%if(rankstr[1][9].equals("1")||admin.getState().equals("1")){ %>
                        <button id="passwordinit" class="btn"><i class="icon-font">&#xe035;</i>初始化登陆密码</button>
                     ...
                     <%} %>
                      <%if(rankstr[1][5].equals("1")||admin.getState().equals("1")){ %>
                         <button id="user_hide" class="btn"><i class="icon-font">&#xe021;</i>隐藏会员</button>
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							<th>用户编号</th><th>用户名</th><th>会员等级</th><th>会员奖衔</th><th>管理级别</th><th>销售商</th><th>销售区域</th><th>服务商</th><th>报单人</th><th>空单名额</th><th>注册时间</th><th>奖金状态</th><th>状态</th><th>操作</th>
						</tr>
						  <%
...
<td><%if(user.getPayTag()==1){%>正常<%}else {%>锁定<%}%></td>
 ...
 <td><a href='admin/UserServlet?method=admin_user_detail&id=<%=user.getUserId()%>'>详情</a>
 &nbsp;|&nbsp;
 <%if(rankstr[1][9].equals("1")||admin.getState().equals("1")){ %>
 <a href='admin/UserServlet?method=admin_user_back&id=<%=user.getId()%>' onclick="if(!confirm('确认要进行退单操作？')) return false;">退单</a>
 <%} %>
 <%if(rankstr[2][9].equals("1")||admin.getState().equals("1")){ %>|<a href='admin/UserServlet?method=admin_user_authority_edit&id=<%=user.getUserId()%>' onclick="if(!confirm('确认要进行权限操作？')) return false;">权限</a>
 <%} %>
 </td>
	 	...
	 	<%}}
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/UserServlet?method=admin_list" method="post">
							<input type="hidden" name="pageNoStr" value="1"/>
							<input type="hidden" name="refereeId" value="<%=StringUtil.notNull(refereeId)%>"/>
							<input type="hidden" name="declarationId" value="<%=StringUtil.notNull(declarationId)%>"/>
							<input type="hidden" name="belongId" value="<%=StringUtil.notNull(belongId)%>"/>
			...
			<form name="pageNoForm"  action="admin/UserServlet?method=admin_list" method="post">
					...
					<a
						href="admin/UserServlet?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&declarationId=<%=StringUtil.notNull(declarationId)%>
						&belongId=<%=StringUtil.notNull(belongId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">首页</a> | <a
						href="admin/UserServlet?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&declarationId=<%=StringUtil.notNull(declarationId)%>
						&belongId=<%=StringUtil.notNull(belongId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/UserServlet?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&declarationId=<%=StringUtil.notNull(declarationId)%>
						&belongId=<%=StringUtil.notNull(belongId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">
                    下一页</a> | <a
						href="admin/UserServlet?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>
						&refereeId=<%=StringUtil.notNull(refereeId)%>&declarationId=<%=StringUtil.notNull(declarationId)%>
						&belongId=<%=StringUtil.notNull(belongId)%>&rankJoin=<%=StringUtil.notNull(rankJoin)%>">尾页</a>
