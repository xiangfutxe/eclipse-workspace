...
String userId = (String) request.getAttribute("userId");
	if (admin == null) {
		...
		if(rankstr[8][12].equals("1")||admin.getState().equals("1")){
%>
		...
		$('#search')
	  	  ...
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/ReportServlet?method=admin_report_export_excel_referee_performance_summary");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">伞下推荐汇总</span>
					
				</div>
			</div>
				<div class="search-wrap">
				<div class="search-content">
					<form action="admin/ReportServlet?method=admin_report_referee_performance_summary" id="searchForm" method="post">
 ...
 <td><%=StringUtil.decimalFormat(wst.getBprice())%></td>
  ...
  <td><%=StringUtil.decimalFormat(wst.getTotalPerformance())%></td>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ReportServlet?method=admin_report_referee_performance_summary" method="post">
			...
			<form name="pageNoForm"  action="admin/ReportServlet?method=admin_report_referee_performance_summary" method="post">
					...
					<a
						href="admin/ReportServlet?method=admin_report_referee_performance_summary&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&userId=<%=StringUtil.notNull(userId)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="admin/ReportServlet?method=admin_report_referee_performance_summary&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&userId=<%=StringUtil.notNull(userId)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ReportServlet?method=admin_report_referee_performance_summary&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>&userId=<%=StringUtil.notNull(userId)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/ReportServlet?method=admin_report_referee_performance_summary&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&userId=<%=StringUtil.notNull(userId)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
