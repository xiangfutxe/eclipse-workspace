	...
	$(document)
	  	  ...
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#myform").attr("action", "admin/AccountServlet?method=exportExcel");
				$("#myform").submit();
	       		 return true;
			}
		 });
...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/AccountServlet?method=admin_money_detail" method="post" id="myform">
							...
							<tr>
								...
								<td><select class="common-text" id="type" name="type">
									...
									<option value="1"
										...
										<option value="2"
											<%if(StringUtil.notNull(type).equals("2")){%>selected<%} %>>--购物券账户--</option>
											<option value="3"
									...
									<option value="汇款充值"
										...
										<option value="报单购物"
											<%if(StringUtil.notNull(tradeType).equals("报单购物")){%>selected<%} %>>--报单购物--</option>
											<option value="升级购物"
											<%if(StringUtil.notNull(tradeType).equals("升级购物")){%>selected<%} %>>--升级购物--</option>
											<option value="会员退单"
											...
											<option value="奖金转报单"
											<%if(StringUtil.notNull(tradeType).equals("奖金转报单")){%>selected<%} %>>--奖金转报单--</option>
											<option value="电子币互转"
											<%if(StringUtil.notNull(tradeType).equals("电子币互转")){%>selected<%} %>>--电子币互转--</option>
											<option value="奖金提现"
											<%if(StringUtil.notNull(tradeType).equals("奖金提现")){%>selected<%} %>>--奖金提现--</option>
											<option value="提现退回"
											...
											<option value="至尊补贴"
											<%if(StringUtil.notNull(tradeType).equals("至尊补贴")){%>selected<%} %>>--至尊补贴--</option>
											<option value="创业金释放"
											<%if(StringUtil.notNull(tradeType).equals("创业金释放")){%>selected<%} %>>--创业金释放--</option>
											<option value="创业购物"
											<%if(StringUtil.notNull(tradeType).equals("创业购物")){%>selected<%} %>>--创业购物--</option>
											<option value="移网导入"
											<%if(StringUtil.notNull(tradeType).equals("移网导入")){%>selected<%} %>>--移网导入--</option>
								</select>
								</td>
							</tr>
							<tr>
 ...
 <tr <%if(t%2==1){%>class="even"<%}%>>
							...
							<tr>
								...
								<br>收支差额：<%=StringUtil.decimalFormat(sum[0]-sum[1])%>
								</td>
							</tr>
						<%}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/AccountServlet?method=admin_money_detail"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/AccountServlet?method=admin_money_detail"
							method="post">
					...
					<a
						href="admin/AccountServlet?method=admin_money_detail&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&type=<%=StringUtil.notNull(type)%>
						...
						href="admin/AccountServlet?method=admin_money_detail&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&type=<%=StringUtil.notNull(type)%>
                 ...
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/AccountServlet?method=admin_money_detail&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/AccountServlet?method=admin_money_detail&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&type=<%=StringUtil.notNull(type)%>
