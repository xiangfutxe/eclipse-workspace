		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				...
				<div class="search-content">
					<form action="admin/ReportServlet?method=admin_report_wsettle_1" id="searchForm" method="post">
       ...
       <td><%=StringUtil.decimalFormat(w.getRtpv()+w.getRtpv1())%></td>
        <td><%=StringUtil.decimalFormat(w.getRtpv()+w.getRpv()+w.getRtpv1()+w.getRpv1())%></td>
			...
			<form name="pageNoForm"  action="admin/ReportServlet?method=admin_report_wsettle" method="post">
					...
					<a
						href="admin/ReportServlet?method=admin_report_wsettle&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/ReportServlet?method=admin_report_wsettle&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ReportServlet?method=admin_report_wsettle&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>&userId=<%=StringUtil.notNull(userId)%>">
                    下一页</a> | <a
						href="admin/ReportServlet?method=admin_report_wsettle&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
