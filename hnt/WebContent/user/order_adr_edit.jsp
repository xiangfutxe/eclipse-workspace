...
<div class="main_bg">
	...
	<div class="second">   
        <div class="scd_m clearfix">
		 ...
		 <div class="scd_mr">
            	<div class="ht_table">
            	<h2>当前位置：订单中心-&gt;订单列表-&gt;<a>新增地址</a></h2>
            	</div>
            		<div class="content content-full-width">
    <form  action="OrderServlet?method=order_adr_update"  id="myform" name="myform" method="post">
  <input type="hidden" name="token" id="token" value="<%=StringUtil.notNull(token)%>"/> 
	 ...
	 <input type="hidden" name="id" id="id" value="<%=orders.getId()%>"/>
                                	...
                                	<div class="notice">
         <h2>修改收货地址</h2><span class="required_notification">* 表示必填项</span>
   </div>
  
  			 <table class ="form-table">
  			 <tr>
  			 <td width="12%"><label><span class="required">*</span>收件人：</label></td>
  			 <td ><input id="receiver" name="receiver"  type="text"  value="<%=StringUtil.notNull(orders.getReceiver())%>">&nbsp;<span id="nameTag"></span></td>
  			 </tr>
  			  <tr>
  			 ...
  			 <td ><input  type="text"  name="address"  id="address" value="<%=StringUtil.notNull(orders.getAddress())%>">&nbsp;<span id="addressTag"></span></td>
  			 </tr>
  			 <tr>
  			 <td><label><span class="required">*</span>联系电话：</label></td>
  			 <td ><input  type="text" name="phone" id="phone" value="<%=StringUtil.notNull(orders.getPhone())%>">&nbsp;<span id="phoneTag" ></span></td>
  			 </tr>
  			 
  		</table>
              <div class="p-li" >
              <input name="button" id="sbtn" type="button" class="button blue-royal" value="提交修改" />
                	<input type="button"  id="reback"  class="button blue-sky" onClick="javascript:history.go(-1)"  value="返回列表">
                	</div>
					</form>
					</div>
         </div> 
...
$("#city_2").citySelect({
   ...
   $("#province").blur(function(){
   		 if($("#province").val()=="")$("#provinceTag").text("省市县选择不完整！").css({"color":"red","font-size":"12px"});
   			else $("#provinceTag").text("");
   			});
   			$("#city").blur(function(){
   		 if($("#city").val()=="")$("#provinceTag").text("省市县选择不完整！").css({"color":"red","font-size":"12px"});
   			else $("#provinceTag").text("");
   			});
