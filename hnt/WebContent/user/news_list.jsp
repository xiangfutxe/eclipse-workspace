...
<div class="main_bg">
        ...
        <div class="scd_m clearfix">
		 ...
		 <div class="scd_mr">
            	...
            	<h2>当前位置：新闻资讯-&gt;<a>最新动态</a></h2>
                	...
                	<div class="notice">
         <h2>新闻资讯</h2><span class="required_notification"><span> </span></a></span>
   </div>
			...
			<table class="result-tab" width="100%">
						<thead><tr>
								<th>序号</th><th>栏目</th><th>标题</th><th>发布时间</th><th>操作</th>
						</tr></thead><tbody>
					 <%
 ...
 <tr>
 <td width="5%"><%=t %></td>
 <td  width="15%"><%=StringUtil.notNull(ca.getSort()) %></td>
 <td><%=StringUtil.notNull(ca.getTitle())%></td>
 <td  width="20%"><%=StringUtil.parseToString(ca.getEntryTime(), DateUtil.yyyyMMddHHmmss)%></td>
  <td>
  <a href="NewsServlet?method=news_detail&id=<%=ca.getId()%>">详情</a></td>
  </tr>
							<%}}
						}else{ %><tr>
              ...
              <div class="pager">
                	 <% if(pager!=null){ %>
                	<span class="left">
                   <form name="pageSizeForm"  action=NewsServlet?method=list method="post" class="add_form">
					...
					<span class="right">
			<form name="pageNoForm"  action="NewsServlet?method=list" method="post"  class="add_form">
						<input type="hidden" name="pageSizeStr" value="<%=pager.getPageSize()%>"/>
					第<font color="red"><%=pager.getPageNo()%>
					</font>页&nbsp; 共<font color="red"><%=pager.getPageCount()%></font>页共<font color="red"><%=pager.getRowCount()%></font>条记录&nbsp;
					<%if(pager.getPageNo()==1) {%> 首页 | 上一页  |<%}else{%>
					<a
						href="NewsServlet?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a> | <a
						href="NewsServlet?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="NewsServlet?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a> | <a
						href="NewsServlet?method=list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
