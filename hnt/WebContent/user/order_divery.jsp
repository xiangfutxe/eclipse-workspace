...
<div class="main_bg">
	...
	<div class="second">   
        <div class="scd_m clearfix">
        	<div class="scd_ml">
				...
				<li class="now"><a href="OrderServlet?method=order_divery">服务订单</a></li> 
         ...
         <h2>支付密码</h2><span class="required_notification"></span>
         	 ...
         	 <table class ="form-table-one">
  			 ...
  			 <td ><input id="psw2" name="psw2"  type="password" value="<%=psw2%>"/>&nbsp;<span id="psw2Tag"></span></td>
                	...
                	<div class="notice">
         <h2>服务订单</h2><span class="required_notification"></span>
   </div>
 
    <div class="p-li ">
     <div class="form-table-one">
         <form  action="OrderServlet?method=order_divery"  id="sform" name="sform" method="post">
    ...
    订单类型：<select name="orderType" id="orderType" >
						...
						</select>&nbsp;订单状态<select name="state" id="state" >
 ...
 <%}else if(order.getOrderType()==5){%>升级购物<%}else if(order.getOrderType()==7){%>代金券购物
  <%}else if(order.getOrderType()==8){%>店铺进货<%}else if(order.getOrderType()==9){%>层碰购物<%}else{%>-<%} %></td>
 ...
 <td><%=StringUtil.parseToString(order.getOrderTime(), DateUtil.yyyyMMddHHmmss)%></td>
 	<td><%if(order.getState()==1){%>待确认<%}else if(order.getState()==2){%>待发货<%}else if(order.getState()==3){%>出库中<%}else if(order.getState()==4){%>已发货<%}else if(order.getState()==5){%>已收货<%}else if(order.getState()==0){%>已退订<%} %></td>
  							<td><a href="OrderServlet?method=order_divery_detail&id=<%=order.getOrderId()%>">详情</a>
	 	...
	 	<%}}
				  ...
				  <div class="pager">
				   <% if(pager!=null){ %>
				  <span class="left">
                   <form name="pageSizeForm"  action="OrderServlet?method=order_divery" method="post" class="add_form">
			...
			<form name="pageNoForm"  action="OrderServlet?method=order_divery" method="post">
					...
					<a
						href="OrderServlet?method=order_divery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="OrderServlet?method=order_divery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="OrderServlet?method=order_divery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="OrderServlet?method=order_divery&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
