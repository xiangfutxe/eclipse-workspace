         	 ...
         	 <table class ="form-table">
  			 ...
  			 <td ><input id="psw2" name="psw2"  type="password" value="<%=psw2%>"/>&nbsp;<span id="psw2Tag"></span></td>
            		...
            		<div class="content content-full-width" id="content-all">
            		  <form  action="OrderServlet?method=order_dmoney_save"  id="myform" name="myform" method="post">
      ...
      <div class="p-li"><p><span style="color:red">*运费说明：自4月16日起，小于200元订单，运费到付！</span></div>
			...
			<select name="addressId" id="addressId">
							...
							%>
									...
									<%} %></select><%}else{ %>
									未设置收货地址，请在<a href="AddressServlet.do?method=address">地址管理</a>中设置好地址信息！<%} %><span id="addressTag"></span></span>
									</div>
								</div>
								<div class="p-li">
		...
		$("#btn1").bind("click", function() {
			$.ajax({
				...
				success : function(data) {
					...
					else if (d.dmoney-totalprice<0 ) $("#msg").text("当前账户余额为"+d.dmoney+"元，账户余额不足，请重新选购！").css({"color":"red","font-size":"9px"});
					else {$("#msg").text("");
...
<%}else{
		out.println("<script>");
		out.println("parent.window.location.href='order_dmoney.jsp';");
		out.println("</script>");
}
}%>