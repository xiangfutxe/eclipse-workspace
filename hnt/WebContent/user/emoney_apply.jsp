         ...
         <h2>支付密码</h2><span class="required_notification"></span>
         	 ...
         	 <table class ="form-table">
  			 ...
  			 <td ><input id="psw2" name="psw2"  type="password" value="<%=psw2%>"/>&nbsp;<span id="psw2Tag"></span></td>
                	...
                	<div class="notice">
         <h2>充值申请</h2><span class="required_notification"><a href="ChargeApplyServlet?method=apply_add">新增申请</a></span>
   </div>
 
    <div class="p-li ">
     <div class="form-table-one">
         <form  action="ChargeApplyServlet?method=apply_list"  id="sform" name="sform" method="post" >
		   
		     <input id="id" name="id"  value='<%=user.getId() %>' type="hidden">
		
    选择状态：<select  id="state"
									name="state">
			...
			<table class="result-tab" width="100%">
						<thead><tr>
							<th>申请编号</th><th>充值金额</th><th>收款账户信息</th><th>充值类型</th><th>备注信息</th><th>申请时间</th><th>状态</th><th>操作</th>
						</tr></thead><tbody>
						 <%
...
<td><%if(ca.getState()==1) {%>待审批<%}else if(ca.getState()==2){ %>
									...
									<td><%if(ca.getState()==1) {%><a href="ChargeApplyServlet?method=apply_del&applyId=<%=StringUtil.notNull(ca.getApplyId()) %>&id=<%=ca.getId() %>" onclick="delcfm();">删除 </a><%} %></td>
 </tr>
	<%}}}else{ %><tr>
				  ...
				  <div class="pager">
				   <% if(pager!=null){ %>
				  <span class="left">
                   <form name="pageSizeForm"  action="ChargeApplyServlet?method=apply_list" method="post" class="add_form">
...
< <span class="right">
			<form name="pageNoForm"  action=ChargeApplyServlet?method=apply_list method="post"  class="add_form">
					...
					<a
						href="ChargeApplyServlet?method=apply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state) %>&typeKey=<%=StringUtil.notNull(typeKey)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="ChargeApplyServlet?method=apply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state) %>&typeKey=<%=StringUtil.notNull(typeKey)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="ChargeApplyServlet?method=apply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&state=<%=StringUtil.notNull(state) %>&typeKey=<%=StringUtil.notNull(typeKey)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="ChargeApplyServlet?method=apply_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state) %>&typeKey=<%=StringUtil.notNull(typeKey)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
