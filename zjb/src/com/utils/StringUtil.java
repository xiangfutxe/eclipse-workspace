...
public static boolean  matchBeforeStr(String str,String match_str){
	boolean b = false;
	String str1[] = str.split(",");
	int index = match_str.length();
	if(index>0){
	for(int i=0;i<str1.length;i++){
		if(str1[i].length()>=4&&str1[i].length()>=index){
			if(str1[i].substring(0, index).equals(match_str)){
				b= true;
				break;
			}
		}
