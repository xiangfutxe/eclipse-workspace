    	...
    	try{
    		Timestamp date = new Timestamp(new Date().getTime());
        		...
        		if(num>1){
    				...
    				double totalBpv = 0;
            		double totalAward = 0;
            		double totalAward1 = 0;
            		//释放业绩
    				for(int i =0;i<wst.length;i++) {
    					if(wst[i]!=null) {
    						if(!StringUtil.notNull(wst[i].getNodeTag()).equals("")){
    						 String[] str = wst[i].getNodeTag().split(",");
    						 String[] str1 = wst[i].getLeftNodePv().split(",");
    						 String[] str1_1 = wst[i].getLeftNodeId().split(",");
    						 String[] str2 = wst[i].getRightNodePv().split(",");
    						 String[] str2_1 = wst[i].getRightNodeId().split(",");
    						 for(int j=0;j<str.length;j++){
    							if(!str[j].equals("")){
    								if(str[j].equals("0")) {
    									if(str1.length>j) {
    										if(!str1[j].equals("")) {
	    										double apv = Double.valueOf(str1[j]);
	    										wst[i].setApv(ArithUtil.add(wst[i].getApv(),apv));
	    										totalApv = ArithUtil.add(totalApv, apv);
    										}
    									}
    									
    									if(str2.length>j) {
    										if(!str2[j].equals("")) {
	    										double bpv = Double.valueOf(str2[j]);
	    										wst[i].setBpv(ArithUtil.add(wst[i].getBpv(),bpv));
	    										totalBpv = ArithUtil.add(totalBpv, bpv);
    										}
    									}
    								}
    							}
            			...
            			if(wst[i]!=null){
            				...
            				if(money>0){
            					...
            					if(realNum>0){
	            					...
	            					totalAward = ArithUtil.add(totalAward, wrd[id].getAmount_4());
	            					wrd[id].setTag(1);
            					}
            				}
            			}
            				...
            				if(wrd[s].getAmount_4()>0){
            					...
            					for(int j=str.length;j>0;j--){
            						if(!str[j-1].equals("")){
            							...
            							totalAward1 = ArithUtil.add(totalAward1, award);
            						}
            						if(t>=9){
	            						j=0;
	            						break;
            	...
            	msg = "释放总业绩为："+StringUtil.decimalFormat(ArithUtil.add(totalApv, totalBpv))
            	+",其中A区业绩："+StringUtil.decimalFormat(totalApv)+"，B区业绩："+StringUtil.decimalFormat(totalBpv)
            			+"；产生总奖金为："+StringUtil.decimalFormat(ArithUtil.add(totalAward, totalAward1))
            			+",其中拓展奖为："+StringUtil.decimalFormat(totalAward)+",培育奖为："+StringUtil.decimalFormat(totalAward1)+"。";
        				}else{
                			msg="奖金参数获取失败。";
                			sqlSession.rollback();
                		}
