...
public class AccountSupplementDao {
    ...
    public String saveAsm(String userId,String adminName,double amount,int type,int payType,String summary,Timestamp date){
    	...
    	String msg = "";
    	try {
    		User user = userMapper.selectMoneyByUserIdForUpdate(userId);
    		if(user!=null){
    		...
    		Integer up1 = asmMapper.save(asm);
	    		if(up1!=null&&up1>0){
	    			...
	    			String tableName ="";
	    			 if(type==1){
						 balance = ArithUtil.add(user.getEmoney(),amount);
						 tableName = Constants.EMONEYDETAIL_TABLE;
						 if(balance<0) msg="账户余额不足，请重新确认。";
						 else{
							 user1.setEmoney(balance);
							 str= "报单券账户"+str;
						 }
	    			 }else  if(type==2){
						 balance = ArithUtil.add(user.getSmoney(),amount);
						 tableName = Constants.SMONEYDETAIL_TABLE;
						 if(balance<0) msg="账户余额不足，请重新确认。";
						 else{
						 user1.setSmoney(balance);
						 str= "购物券账户"+str;
						 }
	    			 }else  if(type==3){
						 balance = ArithUtil.add(user.getDmoney(),amount);
						 tableName = Constants.DMONEYDETAIL_TABLE;
						 if(balance<0) msg="账户余额不足，请重新确认。";
						 else{
						 user1.setDmoney(balance);
						 str= "复消券账户"+str;
						 }
	    			 }else  if(type==4){
