...
public class MoneyDao {
    ...
    public Pager findByPage(AccountDetail ad,String type,Pager pager){
		...
		if(type.equals("1")){
			...
			params.put("tableName", "smoneyDetail");
    ...
    public User  findByAllMoney(){
    	User user = null;
    	try {
    		user = moneyMapper.moneyBalanceByAll();
    		sqlSession.commit();
    	}catch(Exception e){
			e.printStackTrace();
			sqlSession.rollback();
    ...
    public List<AccountDetail>  findByUserId(String tableName,String userId){
    	...
    	try {
    		String sql = "select * from "+tableName+" where userId='"+userId+"' order by entryTime asc";
	...
	public String saveMoney(AccountDetail ad,String type){
    	...
    	try {
    		String tableName ="";
    		if(type.equals("1")){
    			tableName= "emoneyDetail";
    		}else if(type.equals("2")){
    			tableName= "smoneyDetail";
    		}else if(type.equals("3")){
    			tableName= "dmoneyDetail";
    		}else if(type.equals("4")){
    			tableName= "rmoneyDetail";
    		}
    		if(!type.equals("")){
    			Map<String,Object> params = new HashMap<>();
    			params.put("account",ad);
	...
	public String saveEMoneyTransfer(String userId1,String userId2,double amount,Timestamp date){
    	String str = "";
    	try {
    			User user1 = userMapper.selectMoneyByUserIdForUpdate(userId1);
    			User user2 = userMapper.selectMoneyByUserIdForUpdate(userId2);
    			if(user1!=null){
    				if(user2!=null){
