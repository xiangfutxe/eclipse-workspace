			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">电子券管理</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/AccountServlet.do?method=pmoney_list" id="searchForm" method="post" >
			...
			<div class="result-wrap">
				<div class="result-title">
                    <div class="result-list">
                        <a href="admin/AccountServlet.do?method=pmoney_add"><i class="icon-font">&#xe026;</i>电子券支持</a>
                    </div>
                </div>
				<div class="result-content">
					<table class="result-tab" width="99%">
						<tr>
							<th>交易时间</th><th>金额</th><th>有效期</th><th>用户编号</th><th>用户名称</th><th>摘要</th>
 ...
 <td><%=StringUtil.notNull(as.getUserId()) %></td>
  <td><%=StringUtil.notNull(as.getUserName()) %></td>
   <td><%=StringUtil.notNull(as.getSummary()) %></td>
		...
		<%}%>
		<tr><td colspan="6">累计金额：<%=StringUtil.decimalFormat(sum[0]) %>元</td></tr>
		<%}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/AccountServlet.do?method=pmoney_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/AccountServlet.do?method=pmoney_list"
							method="post">
					...
					<a
						href="admin/AccountServlet.do?method=pmoney_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/AccountServlet.do?method=pmoney_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/AccountServlet.do?method=pmoney_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&userId=<%=StringUtil.notNull(userId)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/AccountServlet.do?method=pmoney_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
