		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/InventoryServlet.do?method=apply_out" id="searchForm" method='post'>
							...
							<tr>
								...
								<td><select class="common-text" id="state"
									name="state">
										...
										<option value="2" <%if(StringUtil.notNull(state).equals("2")){%>selected<%}%>>--待质检确认--</option>
										...
										<option value="7" <%if(StringUtil.notNull(state).equals("7")){%>selected<%}%>>--财务审核不通过--</option>
			...
			<div class="result-wrap">
				<div class="result-title">
					<div class="result-list">
						<a href="admin/InventoryServlet.do?method=apply_out_add"><i
...
<th width="120px">申请单号</th>
						...
						if(pager!=null){
							...
							<tr>
								...
								<%}else if(apply.getState()==7) { %>财务审核不通过
								<%} %></td>
								<td><a class="link-update"
									href="admin/InventoryServlet.do?method=apply_out_detail&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									<%if(apply.getState()==1){%>
                              |&nbsp;  <a class="link-del"
											href="admin/InventoryServlet.do?method=apply_out_del&id=<%=StringUtil.notNull(apply.getApplyId())%>" onClick="delcfm()">删除</a>
									<%} %></td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/InventoryServlet.do?method=apply_out"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/InventoryServlet.do?method=apply_out"
							method="post">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/InventoryServlet.do?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&type=<%=type%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_out&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">尾页</a>
