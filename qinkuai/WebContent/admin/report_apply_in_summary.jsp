	...
	$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/ReportServlet.do?method=exportExcel_apply_in_summary");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">入库统计</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/ReportServlet.do?method=report_apply_in_summary"  id="searchForm" method="post">
          ...
          <!--startprint-->
					...
					<div class="result-content">
					 ...
					 <td> <%=StringUtil.notNull(p.getUnit())%></td>
						  ...
						  <td > <%=StringUtil.decimalFormat(p.getNumIn2())%>
						   <td > <%=StringUtil.decimalFormat(p.getNumIn3())%>
						 </td>
						 <td > <%=StringUtil.decimalFormat(p.getNumIn4())%>
						 </td>
 							<td > <%=StringUtil.decimalFormat(p.getNumIn5())%>
						 </td>
						...
						if(StringUtil.notNull(message).equals(""))message="暂无入库信息！";
						%>
						 <tr>
						 <td colspan="10"><%=StringUtil.notNull(message) %></td>
						  </tr>
						 <%} %>
                    </table>
                     </div>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ReportServlet.do?method=report_apply_in_summary" method="post">
			...
			<form name="pageNoForm"  action="admin/ReportServlet.do?method=report_apply_in_summary" method="post">
						<input type="hidden" name="startTime" value="<%=StringUtil.notNull(startTime)%>"/>
							<input type="hidden" name="endTime" value="<%=StringUtil.notNull(endTime)%>"/>
					...
					<a
						href="admin/ReportServlet.do?method=report_apply_in_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a>|<a
						href="admin/ReportServlet.do?method=report_apply_in_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>|<%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ReportServlet.do?method=report_apply_in_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a>|<a
						href="admin/ReportServlet.do?method=report_apply_in_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
				<%}else{ %>下一页|尾页<%} %>&nbsp;跳转到第
				<select
