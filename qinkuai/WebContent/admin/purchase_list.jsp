		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/PurchaseApplyServlet.do?method=admin_list" id="searchForm" method='post'>
			...
			<div class="result-wrap">
						...
						if(pager!=null){
							...
							<tr>
								...
								<%}else if(apply.getState()==3) {%>已审批<%} %></td>								<td><a class="link-update"
									href="admin/PurchaseApplyServlet.do?method=admin_detail&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									
									<%if(apply.getState()==1){%>
                              |&nbsp;  <a class="link-del"
											href="admin/PurchaseApplyServlet.do?method=admin_apply_del&id=<%=StringUtil.notNull(apply.getApplyId())%>" onClick="delcfm()">删除</a>
									<%} %></td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/PurchaseApplyServlet.do?method=admin_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/PurchaseApplyServlet.do?method=admin_list"
							method="post">
					...
					<a
						href="admin/PurchaseApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="admin/PurchaseApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/PurchaseApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/PurchaseApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
