  ...
  <div class="container clearfix">
   <div class="main-wrap">
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span>新增物理仓库</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                <form action="admin/InventoryServlet.do?method=three_save" method="post" id="myform" name="myform">
...
$(function() {
 var reg =/((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/; //电话号码与手机号码同时验证
	...
	$("#inventoryName").blur(function() {
		$.ajax({
				...
				success : function(data) {
					...
					if(d.tag=="0") $("#nameTag").text("物理仓库名称不能为空！").css({"color":"red","font-size":"9px"});
					else if(d.tag=="1")  $("#nameTag").text("物理仓库名称已存在！").css({"color":"red","font-size":"9px"});
					else if(d.tag=="2")  $("#nameTag").text("物理仓库名称可用！").css({"color":"green","font-size":"9px"});
					else $("#nameTag").text("");
					
					if(getByteLen($("#name").val())>50) $("#nameTag").text("物理仓库名称字数不能大于25！").css({"color":"red","font-size":"9px"});
				},
				error : function() {
					alert("系统异常，请稍后重试！");
				}//这里不要加","
	...
	$("#sbtn").click(function(){
		 if (!confirm("确认提交保存物理仓库？")) {
				return false;
			}else{
		var errorstr = "";
		if($("#inventoryName").val()==""){
			$("#nameTag").text("物理仓库名称不能为空！").css({"color":"red","font-size":"9px"}); 
			errorstr ="1";
		}else if(getByteLen($("#name").val)>50){
			$("#nameTag").text("物理仓库名称字数不能大于25！").css({"color":"red","font-size":"9px"});
			errorstr ="1";
		}else if($("#nameTag").text()!="物理仓库名称可用！"){
			errorstr ="1";
		}
