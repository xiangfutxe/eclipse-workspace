		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					...
					<span class="crumb-step">&gt;</span><span class="crumb-name">入库管理</span>
					<span class="crumb-step">&gt;</span><span class="crumb-name">采购审批</span>
				</div>
			</div>
...
<td><%if(apply.getState()==1){%>数据录入中<%}else if(apply.getState()==2) {%>采购审批
								<%}else if(apply.getState()==3) {%>待质检验收<%}else if(apply.getState()==4) {%>待仓管确认
								<%}else if(apply.getState()==5) { %>待财务审核<%}else if(apply.getState()==6) { %>财务已审核
								<%}else if(apply.getState()==7) { %>财务审核不通<%} %></td>								<td><a class="link-update"
									href="admin/InventoryServlet.do?method=apply_in_detail_approve&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									</td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&type=<%=type%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">尾页</a>
