...
<%
	...
	String code = (String)request.getAttribute("supplier_code");
	if (admin == null) {
		out.println("<script>");
	...
	$(function() {
		...
		$("#delAll").click(function() {
			...
			if (!confirm("确认要删除所选店铺？")) {
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_delAll");
				$('#myform').submit();
			}
		 ...
		 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/SupplierServlet.do?method=exportExcel");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
		...
		$("#passwordinit").click(function() {
			...
			var i = 0;
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_psw1_init");
				$('#myform').submit();
					return true;
				}
		...
		$("#password2init").click(function() {
			...
			var i = 0;
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_psw2_init");
				$('#myform').submit();
					return true;
				}
			}
		...
		$("#lock").click(function() {
			...
			var i = 0;
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_lock");
				$('#myform').submit();
					return true;
				}
			}
		...
		$("#unlock").click(function() {
			...
			var i = 0;
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_unlock");
				$('#myform').submit();
					return true;
				}
			}
		...
		$("#add").click(function() {
				...
				$('#myform').attr("action", "admin/SupplierServlet.do?method=admin_add");
				$('#myform').submit();
					return true;
		});
		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						class="crumb-step">&gt;</span><span class="crumb-name">供应商管理</span>
						<span
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/SupplierServlet.do?method=admin_list" id="searchForm"
						method="post">
						<table class="search-tab">
							<tr>
								<td> &nbsp;供应商信息：<input class="common-text" name="userId" id="userId" size="15"
				 ...
				 <div class="result-title">
                    <div class="result-list">
                     <%if(rankstr[8][1].equals("1")||admin.getState().equals("1")){ %>
                        <button id="add" class="btn"><i class="icon-font">&#xe026;</i>新增供应商</button>
                     <%}%>
				...
				<div class="result-content">
					<table class="result-tab" width="100%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							<th>编号</th><th>店名</th><th>联系人</th><th>固定电话</th><th>手机号码</th><th>地址</th><th>状态</th><th>操作</th>
						</tr>
						  <%
...
<td><%=StringUtil.notNull(bh.getPhone())%></td>
...
<td><%if(bh.getState()==1){%>正常<%}else if(bh.getState()==2){%>锁定<%}else{%>-<%} %></td>
 ...
 <td>
 <%if(rankstr[8][2].equals("1")||admin.getState().equals("1")){ %><a href='admin/SupplierServlet.do?method=admin_edit&code=<%=bh.getCode()%>'>修改</a>
 <%} %></td>
	 	...
	 	<%}}
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/SupplierServlet.do?method=admin_list" method="post">
			...
			<form name="pageNoForm"  action="admin/SupplierServlet.do?method=admin_list" method="post">
					...
					<a
						href="admin/SupplierServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&code=<%=StringUtil.notNull(code)%>
						
						">首页</a> | <a
						href="admin/SupplierServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>
						&code=<%=StringUtil.notNull(code)%>
                 ...
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						...
						">
                    下一页</a> | <a
						href="admin/SupplierServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
						&code=<%=StringUtil.notNull(code)%>
						">尾页</a>
