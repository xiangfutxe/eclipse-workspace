...
<body>
	<div class="container clearfix">
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">充值列表</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/ChargeApplyServlet.do?method=admin_list" id="searchForm"
						method="post">
			...
			<div class="result-wrap">
				...
				<div class="result-content">
						...
						if(pager!=null){
							...
							%>
								<tr>
									...
									<td><%if(ca.getState()==2) {%>正常<%}else if(ca.getState()==1){ %>待审批<%}else if(ca.getState()==0){ %>
										已撤回<%} %>
									...
									if(ca.getState()==2) {%>
											<a href="admin/ChargeApplyServlet.do?method=reback&id=<%=ca.getId()%>"
												onclick='cfm("确认撤回该充值记录？撤回后将会从会员账户扣回相应的金额！")'>撤回</a>&nbsp;
										<%}
										} %>
									</td>
								</tr>
							<%}%>
							<tr>
							<td colspan="11" style="text-align:left;">充值合计：<%=StringUtil.decimalFormat(sum[0]) %></td>
						 </tr>
							<%}
						}else{ %><tr>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ChargeApplyServlet.do?method=admin_list" method="post">
			...
			<form name="pageNoForm"  action="admin/ChargeApplyServlet.do?method=admin_list" method="post">
			<input type="hidden" name="state" value="<%=StringUtil.notNull(state)%>"/>
							<input type="hidden" name="typeKey" value="<%=StringUtil.notNull(typeKey)%>"/>
							<input type="hidden" name="userId" value="<%=StringUtil.notNull(userId)%>"/>
							<input type="hidden" name="startTime" value="<%=StringUtil.notNull(startTime)%>"/>
					...
					<a
						href="admin/ChargeApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						...
						href="admin/ChargeApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ChargeApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/ChargeApplyServlet.do?method=admin_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
