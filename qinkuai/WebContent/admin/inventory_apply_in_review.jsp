		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					...
					<span class="crumb-step">&gt;</span><span class="crumb-name">入库财务审核</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/InventoryServlet.do?method=apply_in_review" id="searchForm" method='post'>
						<table class="search-tab">
							<tr>
								...
								<th width="70">单据编号:</th>
								<td><input class="common-text" name="docNum" id="docNum" size="15"
			...
			<div class="result-wrap">
				<div class="result-title">
					<div class="result-list">
						<a href="admin/InventoryServlet.do?method=apply_in_review_add"><i
...
<td><%if(apply.getState()==1){%>数据录入中<%}else if(apply.getState()==2) {%>待质检验收
								...
								<%}else if(apply.getState()==6) { %>财务审核不通过<%} %></td>
									...
									href="admin/InventoryServlet.do?method=apply_in_detail_review&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									</td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/InventoryServlet.do?method=apply_in_review"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/InventoryServlet.do?method=apply_in_review"
							method="post">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>&docNum=<%=StringUtil.notNull(docNum)%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>&docNum=<%=StringUtil.notNull(docNum)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&type=<%=type%>&docNum=<%=StringUtil.notNull(docNum)%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_in_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>&docNum=<%=StringUtil.notNull(docNum)%>">尾页</a>
