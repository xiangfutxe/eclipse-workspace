		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/PurchaseApplyServlet.do?method=admin_review_list" id="searchForm" method='post'>
			...
			<div class="result-wrap">
						...
						if(pager!=null){
							...
							<tr>
								...
								<%}else if(apply.getState()==3) {%>已审批<%} %></td>								<td><a class="link-update"
									href="admin/PurchaseApplyServlet.do?method=admin_review_detail&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
								</td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/PurchaseApplyServlet.do?method=admin_review_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/PurchaseApplyServlet.do?method=admin_review_list"
							method="post">
					...
					<a
						href="admin/PurchaseApplyServlet.do?method=admin_review_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="admin/PurchaseApplyServlet.do?method=admin_review_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/PurchaseApplyServlet.do?method=admin_review_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/PurchaseApplyServlet.do?method=admin_review_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
