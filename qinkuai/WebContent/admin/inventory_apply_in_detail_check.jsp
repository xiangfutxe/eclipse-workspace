...
<span
						...
						<span
						class="crumb-step">&gt;</span><span>入库质检验收</span><span
				 ...
				 <input type="hidden" name="id" id="id" value="<%=ap.getApplyId()%>"/>
                     <table class="search-tab">
                        <tr>
                            <td>
                            <%if(ap.getState()==2){ %>
                            <button class="btn btn-info" id="ybtn"><i class="icon-font">&#xe025;</i>验收通过</button>&nbsp;
                             <button class="btn btn-info" id="nbtn"><i class="icon-font">&#xe024;</i>验收不通过</button>&nbsp;
                             
                  <%} %>
                   <button class="btn" id="b_btn" onclick="javascrip:history.go(-1)"><i class="icon-font">&#xe046;</i>返回验收列表</button>&nbsp;
                   </td>
                     </tr>
                     </table>
                     </form>
...
$(function() {
	...
	$("#ybtn").click(function(){
		if (!confirm("确认通过验收？")) {
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_check_yes");
	...
	$("#nbtn").click(function(){
		if (!confirm("确认验收不通过，不通过入库单将退回上一步流程？")) {
								return false;
							} else {
											$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_check_no");
	...
	$("#b_btn").click(function(){
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_check");
       		 $('#cfmform').submit();
       		 return true;
	});
	});
