...
<body>
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						<span class="crumb-step">&gt;</span><span>配送汇总单</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet.do?method=admin_delivery_summary_list" id="searchForm" method="post">
					...
					<div class="result-content">
						...
						style="font_family:'微软雅黑';color:#000 !important;font-weight:normal;border:0px;border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr style="font-size:16px;">
									<td  align="center">日期：<%=StringUtil.notNull(startTime)%>&nbsp;|&nbsp;<%if(pager!=null){ %>第<%=pager.getPageNo() %>页<%} %></td>
								</tr>
							</tbody>
						</table>
					</div>
 ...
 <tr>
										...
										<td><%=StringUtil.decimalFormat2(nlist[i]) %>
										</td>
										<%
						if(pager!=null){
							Collection coll = pager.getResultList();
							...
							<tr>
							<td rowspan="4" align="center">返回包装统计</td>
   ...
   <%
    ...
    <%
     ...
     <td>
					...
					<div class="result-content">
						<table  style="font_family:'微软雅黑';color:#000 !important;border:0px; font-size:14px; border-collapse:separate; border-spacing:0px 10px;" width="97%" >
							<tbody>
							<tr>
									<td>制表人：</td>
									<td>配货人：</td>
			...
			<form name="pageNoForm"  action="admin/OrderServlet.do?method=admin_delivery_summary_list" method="post">
					...
					<a
						href="admin/OrderServlet.do?method=admin_delivery_summary_list&startTime=<%=StringUtil.notNull(startTime)%>&pageNum=<%=pageNum %>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a>|<a
						href="admin/OrderServlet.do?method=admin_delivery_summary_list&startTime=<%=StringUtil.notNull(startTime)%>&pageNum=<%=pageNum %>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>|<%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet.do?method=admin_delivery_summary_list&startTime=<%=StringUtil.notNull(startTime)%>&pageNum=<%=pageNum %>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a>|<a
						href="admin/OrderServlet.do?method=admin_delivery_summary_list&startTime=<%=StringUtil.notNull(startTime)%>&pageNum=<%=pageNum %>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
				<%}else{ %>下一页|尾页<%} %>
				 &nbsp;<input type="button" class="btn" value="打印配货单"  onclick="preview()" /> 
			</form>
				</div>
                </div>
                <%}%>
