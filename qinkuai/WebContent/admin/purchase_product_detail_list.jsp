...
Product pt = (Product)request.getAttribute("pa_product");
	...
	$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/ReportServlet.do?method=exportExcel_purchase_product_detail");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">单品汇总</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/PurchaseApplyServlet.do?method=admin_purchase_product_detail"  id="searchForm" method="post">
                    <table class="search-tab">
                        <tr>
                          	<th width="100">&nbsp;产品编号：</th><td>
                          	<input class="common-text" type="text" name="productId" id="productId" value="<%=StringUtil.notNull(productId)%>">
                          	起止日期：
								...
								&nbsp;<input class="btn btn-primary btn2" id="search"
									name="search" value="查询" type="button">
									  ...
									  &nbsp;<input type="button" class="btn" value="打印统计表"  onclick="preview()" /> <span
        ...
        <div class="result-wrap">
          <!--startprint-->
					...
					<div class="result-content">
                        ...
                        <%if(pt!=null){ %>
					  	<tr>
								<td><%=StringUtil.notNull(startTime) %><br>至<br><%=StringUtil.notNull(endTime) %></td>				 
						 <td> <%=StringUtil.notNull(pt.getProductType())%> </td>
						  <td> <%=StringUtil.notNull(pt.getProductId())%> </td>
						   <td> <%=StringUtil.notNull(pt.getProductName())%> </td>
						 <td> <%=StringUtil.notNull(pt.getSpecification())%></td>
						 <td> <%=StringUtil.notNull(pt.getUnit())%></td>
						 <td> <%=StringUtil.decimalFormat(pt.getPrice())%></td>
						<td> <%=StringUtil.decimalFormat(pt.getTotalNumIn())%></td>
						<td> <%=StringUtil.decimalFormat(pt.getTotalNumOut())%></td>
						 <td> <%=StringUtil.decimalFormat(pt.getTotalNumIn()-pt.getTotalNumOut())%></td>
						 </tr>
							<%}else{%>
							<tr>
						 <td colspan="10">暂无汇总信息。</td>
						  </tr>
						 <%} %>
                    </table>
                     </div>
