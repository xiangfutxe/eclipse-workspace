  ...
  <div class="container clearfix">
   <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="admin/index.jsp">首页</a><span class="crumb-step">&gt;</span>分店管理<span class="crumb-step">&gt;</span><span>分店修改</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                 <form action="admin/BranchServlet.do?method=admin_edit_save" method="post" id="myform" name="myform">
                    ...
                    <table class="insert-tab" width="100%">
                        <tbody>
                            <tr>
                                ...
                                <td><input type="text" name="code" id="code" value="<%=StringUtil.notNull(branch.getCode()) %>"><span id="codeTag"></span><span id="codeTag1"></span>
                                </td>
                            </tr>
                             <tr>
                                <th width="150px"><i class="require-red">*</i>店铺名称：</th>
                               <td><input
							id="name" name="name" value="<%=StringUtil.notNull(branch.getName()) %>" type="text">&nbsp;<span
							id="nameTag"></span></td>
                            </tr>
                            <tr>
							...
							<option value="1" <%if(branch.getType()==1){ %>selected<%} %>>--加盟店--</option>
							<option value="2" <%if(branch.getType()==2){ %>selected<%} %>>--直营店--</option>
							<option value="3" <%if(branch.getType()==3){ %>selected<%} %>>--外购商--</option>
							</select>&nbsp;<span
							id="typeTag"></span></td>
                            </tr>
                            <tr>
                                ...
                                <td><input
							id="linkname" name="linkman"  type="text" value="<%=StringUtil.notNull(branch.getLinkman()) %>">&nbsp;<span
							id="linknameTag"></span></td>
                            </tr>
                              <tr>
                                <th width="150px"><i class="require-red">*</i>手机号码：</th>
                                <td><input type="text" value="<%=StringUtil.notNull(branch.getTel()) %>"
							name="tel" id="tel">&nbsp;<span id="telTag"></span></td>
                            </tr>
                            <tr>
                                <th width="150px"><i class="require-red"></i>固定电话：</th>
                                <td><input type="text" value="<%=StringUtil.notNull(branch.getPhone()) %>"
                            ...
                            <tr>
                                ...
                                <td><input type="text" value="<%=StringUtil.notNull(branch.getAddress()) %>"
...
$(function() {
	...
	var reg1 = /^[0-9]*$/; //验证非负整数
		...
		prov:"<%=StringUtil.notNull(branch.getProvince())%>", 
    	city:"<%=StringUtil.notNull(branch.getCity())%>",
		dist:"<%=StringUtil.notNull(branch.getArea())%>",
		nodata:"none"
	}); 
		...
		$("#code").blur(
				function() {
					$.ajax({
						type : "post",
						url : "admin/BranchServlet.do?method=codeEditAjax",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
						data : {//设置数据源
							code : $("#code").val(),
							id : $("#id").val()
