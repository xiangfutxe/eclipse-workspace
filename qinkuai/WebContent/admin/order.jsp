...
<body>
		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					...
					<span class="crumb-step">&gt;</span><span class="crumb-name">订单列表</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet.do?method=admin_orders" id="searchForm"
						method="post">
					...
					<option value="1" <%if(StringUtil.notNull(orderType).equals("1")){ %>selected<%} %>>消费购物</option>
						...
						<option value="2"<%if(StringUtil.notNull(state).equals("2")){ %>selected<%} %>>待发货</option>
						<option value="3"<%if(StringUtil.notNull(state).equals("3")){ %>selected<%} %>>出库中</option>
						<option value="4"<%if(StringUtil.notNull(state).equals("4")){ %>selected<%} %>>已发货</option>
				...
				<div class="result-content">
					<table class="result-tab" width="97%">
						<tr>
							...
							<th>订单编号</th><th>订单类型</th><th>总金额</th><th>店铺信息</th><th>收件人</th><th>联系电话</th><th>订单日期</th><th>当前状态</th><th>操作</th>
 ...
 <td><%if(order.getState()==1){%>待确认<%}else if(order.getState()==2){%>待发货<%}else if(order.getState()==3){%>出库中<%}else if(order.getState()==4){%>已发货<%}else if(order.getState()==0){%><span class="font-red">已退订</span><%} %></td>
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet.do?method=admin_orders"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet.do?method=admin_orders"
							method="post">
					...
					<a
						href="admin/OrderServlet.do?method=admin_orders&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>&tag=<%=StringUtil.notNull(tag)%>">首页</a> | <a
						href="admin/OrderServlet.do?method=admin_orders&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>&tag=<%=StringUtil.notNull(tag)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet.do?method=admin_orders&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&userId=<%=StringUtil.notNull(userId)%>&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&tag=<%=StringUtil.notNull(tag)%>">
                    下一页</a> | <a
						href="admin/OrderServlet.do?method=admin_orders&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>&tag=<%=StringUtil.notNull(tag)%>">尾页</a>
