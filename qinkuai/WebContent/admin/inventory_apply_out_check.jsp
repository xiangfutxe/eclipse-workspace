		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					...
					<span class="crumb-step">&gt;</span><span class="crumb-name">出库质检确认</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/InventoryServlet.do?method=apply_out_check" id="searchForm" method='post'>
			...
			<div class="result-wrap">
						...
						if(pager!=null){
							...
							<tr>
								...
								<td><a class="link-update"
									href="admin/InventoryServlet.do?method=apply_out_detail_check&id=<%=StringUtil.notNull(apply.getApplyId())%>">详情</a>&nbsp;
									</td>
							</tr>
						<%}}}else{ %>
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/InventoryServlet.do?method=apply_out_check"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/InventoryServlet.do?method=apply_out_check"
							method="post">
					...
					<a
						href="admin/InventoryServlet.do?method=apply_out_check&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">首页</a> | <a
						href="admin/InventoryServlet.do?method=apply_out_check&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/InventoryServlet.do?method=apply_out_check&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&type=<%=type%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/InventoryServlet.do?method=apply_out_check&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&state=<%=StringUtil.notNull(state)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&type=<%=type%>">尾页</a>
