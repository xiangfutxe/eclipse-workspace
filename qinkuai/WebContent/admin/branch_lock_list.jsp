			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						<span
						class="crumb-step">&gt;</span><span class="crumb-name">锁定分店</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/BranchServlet.do?method=admin_lock_list" id="searchForm"
						method="post">
...
<td><%if(bh.getState()==1){%>正常<%}else if(bh.getState()==2){%>锁定<%}else{%>-<%} %></td>
	 	...
	 	<%}}
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/BranchServlet.do?method=admin_lock_list" method="post">
			...
			<form name="pageNoForm"  action="admin/BranchServlet.do?method=admin_lock_list" method="post">
					...
					<a
						href="admin/BranchServlet.do?method=admin_lock_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&code=<%=StringUtil.notNull(code)%>
						
						">首页</a> | <a
						href="admin/BranchServlet.do?method=admin_lock_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&code=<%=StringUtil.notNull(code)%>
                 ...
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						...
						">
                    下一页</a> | <a
						href="admin/BranchServlet.do?method=admin_lock_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>
						&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&code=<%=StringUtil.notNull(code)%>
						">尾页</a>
