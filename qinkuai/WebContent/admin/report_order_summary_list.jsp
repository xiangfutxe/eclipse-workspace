  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">订单统计</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/ReportServlet.do?method=report_order_summary"  id="searchForm" method="post">
 ...
 Pager pager = (Pager)request.getAttribute("pager");
					 ...
					 <td> <%=StringUtil.notNull(p.getUnit())%></td>
						  ...
						  <td > <%=StringUtil.decimalFormat(p.getTotalNum())%>
						   <td > <%=StringUtil.decimalFormat(p.getTotalPrice())%>
						 </td>
...
double total_num = (Double)request.getAttribute("total_num");
double pre_price = (Double)request.getAttribute("pre_price");
double pre_num = (Double)request.getAttribute("pre_num");
							%>
							<tr><td>合计</td>
							<td>-</td>
							<td>-</td>
							...
							<td>-</td>
							<td>-</td>
							<td><%=StringUtil.decimalFormat(total_num) %></td>
							<td><%=StringUtil.decimalFormat(total_price) %></td>
							<td><%=StringUtil.decimalFormat(pre_num) %></td>
							<td><%=StringUtil.decimalFormat(pre_price) %></td>
							</tr>
							<%}
						}else{ %>
						 <tr>
						 <td colspan="10">暂无销售信息。</td>
						  </tr>
						 <%} %>
                    </table>
                     </div>
				 ...
				 <div class="list-page"> 
                   ...
                   <form name="pageSizeForm"  action="admin/ReportServlet.do?method=report_order_summary" method="post">
			...
			<form name="pageNoForm"  action="admin/ReportServlet.do?method=report_order_summary" method="post">
						<input type="hidden" name="startTime" value="<%=StringUtil.notNull(startTime)%>"/>
							<input type="hidden" name="endTime" value="<%=StringUtil.notNull(endTime)%>"/>
					...
					<a
						href="admin/ReportServlet.do?method=report_order_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a>|<a
						href="admin/ReportServlet.do?method=report_order_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>|<%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/ReportServlet.do?method=report_order_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a>|<a
						href="admin/ReportServlet.do?method=report_order_summary&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
				<%}else{ %>下一页|尾页<%} %>&nbsp;跳转到第
				<select
