...
<script language="JavaScript">
	...
	$(function() {
	 ...
	 $("#all_yes").click(function(){
	  	  if (!confirm("确认批量审核通过所选订单？")) {
				return false;
			}
			else{
	       		$("#myform").attr("action", "admin/OrderServlet.do?method=admin_order_review_all_yes");
				$("#myform").submit();
	       		 return true;
			}
		 });
		 ...
		 $("#all_no").click(function(){
	  	  if (!confirm("确认批量审核退回所选订单？")) {
				return false;
			}
			else{
	       		$("#myform").attr("action", "admin/OrderServlet.do?method=admin_order_review_all_no");
				$("#myform").submit();
	       		 return true;
			}
		 });
		  ...
		  $("#confirm_list").click(function(){
	  	  if (!confirm("确认查看已审核通过的列表？")) {
				return false;
			}
			else{
	       		$("#myform").attr("action", "admin/OrderServlet.do?method=admin_order_review_ylist");
				$("#myform").submit();
	       		 return true;
			}
		 });
...
<body>
			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">出货审核</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderServlet.do?method=admin_order_review" id="searchForm" method="post">
			...
			<form  id="myform" method="post">
				 <div class="result-title">
                    <div class="result-list">
                         <button id="confirm_list" class="btn"><i class="icon-font">&#xe05b;</i>查看审核列表</button>
                      &nbsp;
                        <button id="all_yes" class="btn"><i class="icon-font">&#xe025;</i>批量审核通过</button>
                         &nbsp;
                        <button id="all_no" class="btn"><i class="icon-font">&#xe024;</i>批量审核退回</button>
 ...
 <td><%if(order.getState()==1){%>待确认<%}else if(order.getState()==2){%>待发货<%}else if(order.getState()==3){%>出库中<%}else if(order.getState()==4){%>已发货<%}else if(order.getState()==5){%>已收货<%}else if(order.getState()==0){%>已退订<%} %></td>
  <td><a href="admin/OrderServlet.do?method=admin_order_detail_review&id=<%=order.getOrderId()%>">详情</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderServlet.do?method=admin_order_review"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderServlet.do?method=admin_order_review"
							method="post">
					...
					<a
						href="admin/OrderServlet.do?method=admin_order_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderServlet.do?method=admin_order_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderServlet.do?method=admin_order_review&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
