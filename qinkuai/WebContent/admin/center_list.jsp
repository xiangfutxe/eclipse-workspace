...
<body>
		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/CenterServlet.do?method=list" id="searchForm"
						method="post">
				...
				<div class="result-content">
					<table class="result-tab" width="97%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							<th>中心编号</th><th>中心类型</th><th>中心名称</th><th>联系人信息</th><th>联系电话</th><th>经营区域</th><th>经营地址</th><th>营业执照</th><th>当前状态</th><th>操作</th>
						</tr>
						 <%
 ...
 <tr>
  ...
  <td><%if(center.getType()==1){%>
								社区店<%}else if(center.getType()==2){%>中心店<%}%></td>
...
<td><%=StringUtil.notNull(center.getPhone()) %></td>
 ...
 <td><a  href="admin/CenterServlet.do?method=admin_detail&id=<%=center.getId() %>">详情</a>
 |<a  href="admin/CenterServlet.do?method=admin_edit&id=<%=center.getId() %>">修改</a>
 <%if(center.getState()==2){%>|<a  href="admin/CenterServlet.do?method=admin_up&id=<%=center.getId() %>">类型</a>
|<a  href="admin/CenterServlet.do?method=admin_close&id=<%=center.getId() %>" onclick="javascript:if(!confirm('确认要关闭店铺？')) return false;">关闭</a><%} %>
<%if(center.getState()==0){%>|<a  href="admin/CenterServlet.do?method=admin_open&id=<%=center.getId() %>" onclick="javascript:if(!confirm('确认要重新开通店铺？')) return false;">开通</a><%} %></td>
