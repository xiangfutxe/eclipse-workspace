			...
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.action">首页</a><span
						class="crumb-step">&gt;</span><span class="admin/InventoryServlet.do?method=apply_in">入库管理</span>
						<span class="crumb-step">&gt;</span><span class="crumb-name">采购审批</span>
										<span class="crumb-step">&gt;</span><span class="crumb-name">入库单</span>
					 ...
					 <td> <%=StringUtil.notNull(p.getSpecification())%></td>
					  <td> <%=StringUtil.notNull(p.getUnit())%></td>
					   <td> <%=StringUtil.decimalFormat(p.getNum())%></td>
						<td><%if(ap.getType()==1){%> <%=StringUtil.decimalFormat(p.getPrice())%><%} %></td>
						<td><%if(ap.getType()==1){%> <%=StringUtil.decimalFormat(p.getTotalPrice())%><%} %></td>
						<td><%=StringUtil.notNull(p.getRemark()) %>
							</tr>
					<%}
					if(coll.size()<6){
					for(int i=t;i<6;i++){%>
				 ...
				 <input type="hidden" name="id" id="id" value="<%=ap.getApplyId()%>"/>
                     <table class="search-tab">
                        <tr>
                            <td>
                            <%if(ap.getState()==2){ %>
                            <button class="btn btn-info" id="ybtn"><i class="icon-font">&#xe025;</i>审核通过</button>&nbsp;
                             <button class="btn btn-info" id="nbtn"><i class="icon-font">&#xe024;</i>审核不通过</button>&nbsp;
                             
                  <%} %>
                   <button class="btn" id="b_btn" ><i class="icon-font">&#xe046;</i>返回审核列表</button>&nbsp;
                   </td>
                     </tr>
                     </table>
                     </form>
...
$(function() {
	...
	$("#ybtn").click(function(){
		if (!confirm("确认采购审核通过？")) {
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_approve_yes");
	...
	$("#nbtn").click(function(){
		if (!confirm("确认采购审核不通过，不通过入库单将退回上一步流程？")) {
								return false;
							} else {
											$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_approve_add");
	...
	$("#b_btn").click(function(){
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_in_approve");
       		 $('#cfmform').submit();
       		 return true;
	});
	});
