	...
	$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/ReportServlet.do?method=exportExcel_apply_out_summary");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">出库统计</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/ReportServlet.do?method=report_apply_out_summary"  id="searchForm" method="post">
          ...
          <!--startprint-->
					...
					<div class="result-content">
					 ...
					 <td> <%=StringUtil.notNull(p.getUnit())%></td>
						 <td > <%=StringUtil.decimalFormat(p.getNumOut1())%></td>
						  <td > <%=StringUtil.decimalFormat(p.getNumOut2())%></td>
						   <td > <%=StringUtil.decimalFormat(p.getNumOut3())%>
						 </td>
						 <td > <%=StringUtil.decimalFormat(p.getNumOut4())%>
						 </td>
 							<td > <%=StringUtil.decimalFormat(p.getNumOut5())%>
						 </td>
						 <td > <%=StringUtil.decimalFormat(p.getNumOut6())%>
						 </td>
						 <td > <%=StringUtil.decimalFormat(p.getNumOut7())%>
						 </td>
						 <td > <%=StringUtil.decimalFormat(p.getNumOut8())%>
						 </td>
						  <td > <%=StringUtil.decimalFormat(p.getNumOut9())%>
						 </td>
						...
						if(StringUtil.notNull(message).equals(""))message="暂无出库信息！";
						%>
						 <tr>
						 <td colspan="15"><%=StringUtil.notNull(message) %></td>
						  </tr>
						 <%} %>
                    </table>
                     </div>
