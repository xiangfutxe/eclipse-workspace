					...
					<table width="100%" style="border:0px;font-size:12px; border-collapse:separate; border-spacing:0px 10px;">
					 ...
					 <td> <%=StringUtil.notNull(p.getSpecification())%></td>
					  <td> <%=StringUtil.notNull(p.getUnit())%></td>
						...
						<td><a href="admin/InventoryServlet.do?method=apply_out_cfm_edit&id=<%=p.getId()%>">修改</a></td>
							</tr>
					<%}
					if(coll.size()<6){
					for(int i=t;i<6;i++){%>
                        ...
                        <tr>
                            ...
                            <button class="btn btn-info" id="ybtn"><i class="icon-font">&#xe025;</i>确认通过</button>&nbsp;
                             <button class="btn btn-info" id="nbtn"><i class="icon-font">&#xe024;</i>确认不通过</button>&nbsp;
                              <%}else if(ap.getState()==7){ %>
                            <button class="btn btn-info" id="ybtn"><i class="icon-font">&#xe025;</i>重新提交</button>&nbsp;
...
$(function() {
	...
	$("#ybtn").click(function(){
		if (!confirm("确认通过？")) {
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_out_cfm_yes");
	...
	$("#nbtn").click(function(){
		if (!confirm("确认不通过，不通过出库单将退回上一步流程？")) {
								return false;
							} else {
											$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_out_cfm_no");
	...
	$("#b_btn").click(function(){
				...
				$('#cfmform').attr("action","admin/InventoryServlet.do?method=apply_out_cfm");
       		 $('#cfmform').submit();
       		 return true;
	});
	});
