...
<body>
		...
		<div class="main-wrap">
			<div class="crumb-wrap">
				<div class="crumb-list">
					<i class="icon-font"></i><a href="admin/index.jsp">首页</a><span
						...
						class="crumb-step">&gt;</span><span class="crumb-name">配货确认</span>
				</div>
			</div>
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/OrderCenterServlet?method=admin_order_divery_confirm" id="searchForm" method="post">
							<table class="search-tab">
							<tr>
							<td>
								店铺编号：<input class="common-text" name="userId" id="userId" size="15"
			...
			<div class="result-title">
					<div class="result-list">
						<a href="javascript:void(0)"  onclick="submit_confirm();"><i
							class="icon-font">&#xe05b;</i>批量确认配货</a>
					</div>
				</div>
					<form action="admin/OrderCenterServlet?method=admin_order_divery_confirm_yes" id="myform" method="post">
				...
				<div class="result-content">
					<table class="result-tab" width="99%">
						...
						<th class="tc" width="5%"><input class="allChoose" id="ckAll" name="ckAll" type="checkbox"></th>
							...
							<th>订单编号</th><th>订单类型</th><th>总金额</th><th>分店信息</th><th>收件人</th><th>联系电话</th><th>订单日期</th><th>当前状态</th><th>配货状态</th><th>操作</th>
 ...
 <td><%=StringUtil.notNull(order.getOrderId()) %></td>
  <td><%if(order.getOrderType()==1){ %>报单购物<%}else if(order.getOrderType()==2){ %>复消购物<%}else if(order.getOrderType()==5){%>升级购物<%}else{%>-<%} %></td>
 ...
 <td><%if(order.getDiveryTag()==0){%>待配货<%}else if(order.getDiveryTag()==1){%>已配货<%}else if(order.getDiveryTag()==2){%>配货已确认<%} %></td>
 <td><a href="admin/OrderCenterServlet?method=admin_order_detail_divery_confirm&id=<%=order.getOrderId()%>">详情</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/OrderCenterServlet?method=admin_order_divery_confirm"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/OrderCenterServlet?method=admin_order_divery_confirm"
							method="post">
					...
					<a
						href="admin/OrderCenterServlet?method=admin_order_divery_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">首页</a> | <a
						href="admin/OrderCenterServlet?method=admin_order_divery_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/OrderCenterServlet?method=admin_order_divery_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
                    ...
                    下一页</a> | <a
						href="admin/OrderCenterServlet?method=admin_order_divery_confirm&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&orderType=<%=StringUtil.notNull(orderType)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&userId=<%=StringUtil.notNull(userId)%>">尾页</a>
