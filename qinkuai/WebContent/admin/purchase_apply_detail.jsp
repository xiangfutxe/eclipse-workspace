			...
			<div class="result-wrap">
					 ...
					 <td> <%=StringUtil.notNull(p.getSpecification())%></td>
					  <td> <%=StringUtil.notNull(p.getUnit())%></td>
					   <td> <%=p.getNum()%></td>
						...
						<td> <%if(ap.getState()==1){ %>
						 <a href="admin/PurchaseApplyServlet.do?method=admin_apply_pro_del&id=<%=p.getId() %>" onclick="javascript:if (!confirm('确认删除产品信息？')) return false;">删除</a>
						 <%} %>
						 </td>
							</tr>
					<%}%>
				...
				<p>&nbsp;</p>
				 <div class="result-list">
				 	<form action="admin/PurchaseApplyServlet.do?method=admin_apply_pro_cfm" id="cfmform" method="post">
				 <input type="hidden" name="id" id="id" value="<%=ap.getApplyId()%>"/>
                  ...
                  <%} %>
                   <button class="btn" id="b_btn"><i class="icon-font">&#xe046;</i>返回采购申请</button>&nbsp;
                   </td>
                     </tr>
                     </table>
                     </form>
...
<p>&nbsp;</p>
				...
				<div class="result-content">
									<form action="admin/PurchaseApplyServlet.do?method=admin_apply_pro_add" id="addform" method="post">
...
$(function() {
	...
	$("#sbtn").click(function(){
		if (!confirm("确认提交采购申请？")) {
			...
			$('#cfmform').attr("action","admin/PurchaseApplyServlet.do?method=admin_apply_pro_cfm");
