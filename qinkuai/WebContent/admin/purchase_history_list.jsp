	...
	$("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "admin/PurchaseApplyServlet.do?method=exportExcel_history_search");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            ...
            <span class="crumb-step">&gt;</span><span class="crumb-name">历史采购</span></div>
        </div>
        <div class="search-wrap">
            <div class="search-content">
                <form action="admin/PurchaseApplyServlet.do?method=admin_history_search"  id="searchForm" method="post">
          ...
          <!--startprint-->
					...
					<div class="result-content">
						...
						style="font_family:'微软雅黑';color:#000 !important;font-weight:normal;border:0px;border-collapse:separate; border-spacing:0px 10px;">
							...
							<strong>历史采购明细汇总表</strong>
							<br>
							</td>
							</tr>
							<tr  style="font-size:14px;">
							<td>编号：<%=pt.getProductId() %></td>
							<td>货品编号：<%=pt.getProductName() %></td>
							<td>规格：<%=pt.getSpecification() %></td>
							<td>单位：<%=pt.getUnit() %></td>
								</tr>
								<tr style="font-size:14px;">
								<td colspan="2">总入库数：<%=StringUtil.decimalFormat(pt.getTotalNum()) %></td>
									<td colspan="2">统计日期：<%=StringUtil.notNull(startTime)%>&nbsp;至&nbsp;<%=StringUtil.notNull(endTime)%></td>
								</tr>
							</tbody>
						</table>
					</div>
 ...
 Pager pager = (Pager)request.getAttribute("pager");
						if(pager!=null){
							...
							if(coll!=null){
									...
									int t=0;
									while(it1.hasNext()){
									PurchaseApply p = (PurchaseApply)it1.next();
									t++;
							%>
					  	<tr>
							<td> <%=t%></td>						 
						 <td> <%=StringUtil.notNull(p.getApplyId())%> </td>
						 <td> <%=StringUtil.notNull(p.getDocNum())%></td>
						 <td> <%=StringUtil.decimalFormat(p.getPrice())%></td>
						<td> <%=StringUtil.decimalFormat(p.getNum())%></td>
						 
						  <td> <%=StringUtil.notNull(p.getSupplierName())%> </td>
						
 							<td > <%=StringUtil.parseToString(p.getEntryTime(), DateUtil.yyyyMMddHHmmss)%>
						 </td>
                                <td > <%=StringUtil.parseToString(p.getReviewTime(), DateUtil.yyyyMMddHHmmss)%>
						 </td>
						 </tr>
							<%}}else{%>
							<tr>
						 <td colspan="9">暂无采购信息。</td>
						  </tr>
