...
public class MessageServlet extends HttpServlet {
			...
			if(db.createConn()){
				...
				if(!(startTimeStr.equals("")||endTimeStr.equals(""))){
					...
					String  endTime = endTimeStr+" 23:59:59";
							sql ="select * from message where entryTime>='"+startTime+"' and entryTime<='"+endTime+"' and (userId like'%"+userId+"%' or userName like'%"+userId+"%')  and state='1' order by id desc";
						
					}else{
						sql ="select * from message where  userId like'%"+userId+"%' or userName like'%"+userId+"%'  and state='1' order by id desc";
						}
				stmt= db.getStmtread();
	...
	public void list(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			if(user!=null){
			if(db.createConn()){
				String sql  ="select * from message where  userId ='"+user.getCode()+"' and state='1' order by entryTime desc";
				stmt= db.getStmtread();
				rs =stmt.executeQuery(sql);
				double[] sum = {0};
				while(rs.next()){
	...
	public void add(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		Branch branch = (Branch)request.getSession().getAttribute("sys_branch");
			if(branch!=null){
				int token = (int)(1+Math.random()*(1000000-1+1));
				request.getSession().setAttribute("token", String.valueOf(token));
					RequestDispatcher dispatcher = request.getRequestDispatcher("msg_add.jsp");
					dispatcher.forward(request, response);
				}else{
					request.setAttribute("message", "会员未登陆，请登陆！");
					RequestDispatcher dispatcher = request.getRequestDispatcher("msg_message.jsp");
					dispatcher.forward(request, response);
			}
		}
		...
		try {
			if(branch!=null){
				if(StringUtil.notNull(token_old).equals(token)){
					...
					else if(userMsg.equals("")){ error = "留言内容不能为空！";}
			if(error.equals("")){
			if(db.createConn()){
				 conn = db.getConnection();
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 branch.getCode()+"','"+branch.getName()+"','"+userMsg+"','1','"+date+"')";
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
						 request.setAttribute("message", "留言发布成功，请在留言信箱查看留言信息！");
	...
	public void del(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException, SQLException{
		...
		try {
			...
			if(db.createConn()){
				 ...
				 boolean autoCommit= conn.getAutoCommit();
				 conn.setAutoCommit(false);
						 String sql = "update message set state='0' where id='"+id+"'"; 
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
						 request.setAttribute("message", "留言信息删除成功，请在留言信箱查看留言信息！");
		...
		try {
			if(admin!=null){
				...
				if(StringUtil.notNull(token_old).equals(token)){
					...
					else if(adminMsg.equals("")){ error = "留言内容不能为空！";}
			if(error.equals("")){
			if(db.createConn()){
				 conn = db.getConnection();
				 boolean autoCommit= conn.getAutoCommit();
