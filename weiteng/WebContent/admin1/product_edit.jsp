...
Integer[][] attributeValueTag = (Integer[][])request.getAttribute("attributeValueTag");
  ...
  <div class="container clearfix">
   <div class="main-wrap">
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="admin/index.jsp">首页</a><span class="crumb-step">&gt;</span><a class="crumb-name" href="admin/ProductServlet?method=admin_product_list">产品列表</a><span class="crumb-step">&gt;</span><span>修改产品</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                <form action="admin/ProductServlet?method=admin_product_update" method="post" id="myform" name="myform">
                  ...
                  <input type="hidden" name="id" id="id" value="<%=product.getId()%>"/> 
                    <table class="insert-tab" width="97%">
                        <tbody><tr>
                            ...
                            <td>
                                   <input name="productType" type="hidden" value="<%=product.getType()%>"/>
                                   <%=product.getType()%>
                            </td>
                        </tr>
                       <tr>  <th><i class="require-red">*</i>产品类型：</th><td>
                       <select class="common-text" id="isGroup" name="isGroup">
                        <option value='1' <%if(product.getIsGroup()==1) {%>selected<%} %>>单品</option>
                       </select><span id="typeTag1"></span>
                       </td></tr>
                        <tr>  <th><i class="require-red">*</i>所属专区：</th><td>
                         <% String[] str = product.getBelongArea().split(",");%>
                                    <input name="belongArea" type="checkbox" value="报单专区"
                                   <%for(int i=0;i<str.length;i++){ if(str[i].equals("报单专区")){%>checked="checked"<% } }%>/>
                                   报单专区
                                    <input name="belongArea" type="checkbox" value="积分专区"  <%for(int i=0;i<str.length;i++){ if(str[i].equals("积分专区")){ %>checked="checked"<% } }%>/>
                                  	积分专区
                    <span id="dTypeTag"></span>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input class="common-text required" id="code" name="code"  value="<%=product.getCode() %>" type="text" readonly>&nbsp;<span id="codeTag"></span><span id="codeTag1"></span>
                                </td>
                            </tr>
                            <tr>
                                <th><i class="require-red">*</i>产品名称：</th>
                                <td><input class="common-text" name="productName" id="productName" type="text"  value="<%=product.getName()%>">&nbsp;<span id="nameTag"></span></td>
                            </tr>
                              <tr>
                                <th><i class="require-red">*</i>产品规格：</th>
                                <td><input class="common-text" name="spec" id="spec" type="text"  value="<%=product.getSpec()%>">&nbsp;<span id="specTag"></span></td>
                            </tr>
                            ...
                            if(attribute!=null){
                             for(int i=0;i<attribute.length;i++){ %>
                              <tr>
                                ...
                                <input class="common-text" name="feature<%=attributeId[i] %>" value="<%=StringUtil.notNull(attributeValue[i][0]) %>" type="text">
                                <%}else {%>
                                	<% for(int j=0;j<attributeValue[i].length;j++){ 
                                	if(!StringUtil.notNull(attributeValue[i][j]).equals("")){
                                	%>
                                	  <input class="common-text" name="feature<%=attributeId[i] %>" value="<%=StringUtil.notNull(attributeValue[i][j])%>" type="checkbox" <%if(attributeValueTag[i][j]!=null){if(attributeValueTag[i][j]==1){ %>checked="checked"<%} }%>>&nbsp;<%=StringUtil.notNull(attributeValue[i][j]) %>&nbsp;
                                	<%}
                                	} %>
                                <%} %>
                               </td>
                            ...
                            <tr>
                                ...
                                <td><input class="common-text" name="price" id="price" type="text" value="<%=product.getPrice()%>">&nbsp;<span id="priceTag"></span></td>
                            </tr>
                             
                              <tr>
                                <th><i class="require-red">*</i>产品PV值：</th>
                                <td><input class="common-text" name="pv" id="pv" type="text" value="<%=product.getPv()%>">&nbsp;<span id="pvTag"></span></td>
                            </tr>
                            <tr>
                                <th><i class="require-red">*</i>所需积分：</th>
                                <td>
                                <input class="common-text" name="integral" id="integral" type="text" value='<%=product.getIntegral() %>' >&nbsp;<span id="integralTag"></span>
                            </tr>
                           ...
                           <tr>
                            ...
                            <tr>
                                <th></th>
                                <td>
                                    <input id="sbtn" class="btn btn-primary mr10" value="提交修改" type="button">
