...
<body>
   ...
   <form  id ="myform" method="post">
    ...
    <table class="table-detail">
        ...
        <td>订单类型</td>
		<td><c:if test="${orders.orderType==1}">等级购物</c:if>
          <c:if test="${orders.orderType==2}">零售购物</c:if>
          <c:if test="${orders.orderType==3}">特买购物</c:if>
          <c:if test="${orders.orderType==4}">积分购物</c:if>
          </td>
								<td>支付方式</td><td>
					<c:if test="${orders.payType==1}">转账汇款</c:if>
          <c:if test="${orders.payType==2}">向上级支付</c:if>
								</td>
								<td>当前状态</td>
								<td><c:if test="${orders.state==0}">录入中</c:if>
          <c:if test="${orders.state==1}">待支付</c:if>
          <c:if test="${orders.state==2}">已支付</c:if></td>
							</tr>
							<tr>
								<td>会员信息
								</td>
    ...
    <div  class="tabson">
     <ul class="forminfo">
        <li>
                <input id="yes_click"  type="button" class="btn btn-green" value="确认收款"/>
...
$(document).ready(function(){
  ...
  $(".save_sure").click(function(){
	  $(".save_tip").fadeOut(100);
		...
		$('#myform').attr("action", "OrderServlet?method=admin_order_review_yes");
		 	$('#myform').submit();
