...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_orders_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_orders_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","OrderDeliveryServlet?method=admin_orders");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="OrderDeliveryServlet?method=admin_orders" method="post">
    <ul class="seachform">
	...
	<option value="1" <c:if test="${state=='1' }">selected</c:if>>--待确认--</option>
	<option value="2" <c:if test="${state=='2' }">selected</c:if>>--待发货--</option>
	<option value="3" <c:if test="${state=='3' }">selected</c:if>>--已发货--</option>
	<option value="4" <c:if test="${state=='4' }">selected</c:if>>--已签收--</option>
   </select>
     </li>
    <li><label>购物类型:</label>
   ...
   <option value="3" <c:if test="${orderType=='3' }">selected</c:if>>--积分购物--</option>
    ...
    <table class="tablelist">
        ...
        <td>${result.orderId}</td>
         ...
         <td>${result.price}</td>
           <td>${result.province}${result.city}${result.area}${result.address}</td>
        <td><fmt:formatDate value="${result.orderTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
        <td><c:if test="${result.state==0}">已删除</c:if>
         <c:if test="${result.state==1}">待确认</c:if>
          <c:if test="${result.state==2}">待发货</c:if>
          <c:if test="${result.state==3}">已发货</c:if>
          <c:if test="${result.state==4}">已签收</c:if>
         </td>
        <td><a href="OrderDeliveryServlet?method=admin_order_detail&id=${result.orderId}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="OrderDeliveryServlet?method=admin_orders" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_orders&orderId=${orderId}&userId=${userId}&state=${state}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_orders&orderId=${orderId}&userId=${userId}&state=${state}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_orders&orderId=${orderId}&userId=${userId}&state=${state}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_orders&orderId=${orderId}&userId=${userId}&state=${state}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
