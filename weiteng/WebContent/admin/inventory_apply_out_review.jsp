...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[4][6].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-出库审核</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","InventoryServlet?method=apply_out_review_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","InventoryServlet?method=apply_out_review_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","InventoryServlet?method=apply_out_review");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="InventoryServlet?method=apply_out_review" method="post">
    <ul class="seachform">
    <li><label>出库单号：</label><input name="applyId" type="text" class="scinput" value="${productName}" /></li>
     <li><label>状态:</label>
	...
	<option value="1"  <c:if test="${type=='1' }">selected</c:if>>--加工出库--</option>
     <option value="2"  <c:if test="${type=='2' }">selected</c:if>>--损耗出库--</option>
    <option value="3" <c:if test="${type=='3' }">selected</c:if>>--外销出库--</option>
   <option value="4" <c:if test="${type=='4' }">selected</c:if>>--体验出库--</option>
     <option value="5" <c:if test="${type=='5' }">selected</c:if>>--赠送出库--</option>
     <option value="6" <c:if test="${type=='6' }">selected</c:if>>--研发出库--</option>
    <option value="7" <c:if test="${type=='7' }">selected</c:if>>--退货出库--</option>
    <option value="8" <c:if test="${type=='8'}">selected</c:if>>--盘亏出库--</option>
    ...
    <table class="tablelist">
        ...
        <td>${result.applyId}</td>
         <td>${result.inventory}</td>
          <td><c:if test="${result.type==1}">加工出库</c:if>
          <c:if test="${result.type==2}">损耗出库</c:if>
          <c:if test="${result.type==3}">外销出库</c:if>
          <c:if test="${result.type==4}">体验出库</c:if>
            <c:if test="${result.type==5}">赠送出库</c:if>
              <c:if test="${result.type==6}">研发出库</c:if>
                <c:if test="${result.type==7}">退货出库</c:if>
                  <c:if test="${result.type==8}">盘亏出库</c:if>
         </td>
        ...
        <td><a href="InventoryServlet?method=apply_out_review_detail&id=${result.applyId}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="InventoryServlet?method=apply_out_review" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="InventoryServlet?method=apply_out_review&applyId=${applyId}&state=${state}&type=${type}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="InventoryServlet?method=apply_out_review&applyId=${applyId}&state=${state}&type=${type}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="InventoryServlet?method=apply_out_review&applyId=${applyId}&state=${state}&type=${type}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="InventoryServlet?method=apply_out_review&applyId=${applyId}&state=${state}&type=${type}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
