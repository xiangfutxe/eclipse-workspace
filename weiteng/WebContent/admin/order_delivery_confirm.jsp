...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[5][3].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-配货确认</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_order_confirm_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_order_confirm_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","OrderDeliveryServlet?method=admin_order_confirm");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="OrderDeliveryServlet?method=admin_order_confirm" method="post">
    <ul class="seachform">
	...
	<option value="1"  <c:if test="${orderType=='1' }">selected</c:if>>--等级购物--</option>
   ...
   <option value="3" <c:if test="${orderType=='3' }">selected</c:if>>--积分购物--</option>
    ...
    <table class="tablelist">
        ...
        <td><c:if test="${result.state==0}">已删除</c:if>
          ...
          <c:if test="${result.state==3}">已签收</c:if>
         </td>
        <td><a href="OrderDeliveryServlet?method=admin_order_detail_confirm&id=${result.orderId}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="OrderDeliveryServlet?method=admin_order_confirm" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_order_confirm&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_order_confirm&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_order_confirm&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_order_confirm&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
