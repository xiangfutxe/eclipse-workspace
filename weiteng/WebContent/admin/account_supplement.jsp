...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[6][6].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-账户增扣</title>
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","AccountSupplementServlet?method=list_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","AccountSupplementServlet?method=list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="AccountSupplementServlet?method=list" method="post">
    <ul class="seachform">
   ...
   <option value="3" <c:if test="${type=='3' }">selected</c:if>>--代金券账户--</option>
	...
	<option value="1"  <c:if test="${payType=='1' }">selected</c:if>>--补增--</option>
    <option value="2" <c:if test="${payType=='2' }">selected</c:if>>--补扣--</option>
   </select>
    </li>
    </ul>
          <ul class="seachform">                      
    ...
    <table class="tablelist">
        ...
        <td>${status.index+1}</td>
         <td>${result.userId}（${result.userName}）</td>
          <td>${result.amount}</td>
          <td><c:if test="${result.type==1}">佣金账户</c:if>
          <c:if test="${result.type==2}">特卖权账户</c:if>
          <c:if test="${result.type==3}">代金券账户</c:if>
            <c:if test="${result.type==4}">积分账户</c:if>
         </td>
          <td><c:if test="${result.payType==1}">补增</c:if>
          <c:if test="${result.payType==2}">补扣</c:if>
         </td>
         <td>${result.admin}</td>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="AccountSupplementServlet?method=list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="AccountSupplementServlet?method=list&type=${type}&userId=${userId}&payType=${payType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="AccountSupplementServlet?method=list&type=${type}&userId=${userId}&payType=${payType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="AccountSupplementServlet?method=list&type=${type}&userId=${userId}&payType=${payType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="AccountSupplementServlet?method=list&type=${type}&userId=${userId}&payType=${payType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
