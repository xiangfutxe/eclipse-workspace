...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[7][0].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-佣金查询</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","RewardDetailServlet?method=admin_list_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","RewardDetailServlet?method=admin_list_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","RewardDetailServlet?method=admin_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="RewardDetailServlet?method=admin_list" method="post">
    <ul class="seachform">
   ...
   <li><label>会员信息</label><input name="userId" type="text" class="scinput" value="${userId}" /></li> 
    ...
    <table class="tablelist">
        ...
        <td>${result.userId}</td>
         ...
         <td><c:if test="${result.type==0}">-</c:if>
          <c:if test="${result.type==1}">分销奖</c:if>
          <c:if test="${result.type==2}">团队奖</c:if>
          <c:if test="${result.type==3}">分红奖</c:if>
          <c:if test="${result.type==4}">区域奖</c:if></td>
          <td>${result.award}</td>
         <td>${result.refereeUserId}(${result.refereeUserName})</td>
             <td>${result.amount}</td>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="RewardDetailServlet?method=admin_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="RewardDetailServlet?method=admin_list?userId=${userId}&state=${state}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="RewardDetailServlet?method=admin_list&userId=${userId}&state=${state}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="RewardDetailServlet?method=admin_list&userId=${userId}&state=${state}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="RewardDetailServlet?method=admin_list&userId=${userId}&state=${state}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
