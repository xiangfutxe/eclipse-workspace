...
<body>
    ...
    <div class="formtitle"><span>基本信息</span></div>
     ...
     <form action="ProductTypeServlet?method=updateProductType" id="myform" method="post">
      <input type="hidden" name="id" id="id" value="${productType.id }"/> 
                 <input type="hidden" name="token" id="token" value="${sessionScope.token }"/> 
    <ul class="forminfo">
    <li><label>分类名称<b>*</b></label>
    <input name="typeName" id="typeName" type="text" class="dfinput" value="${productType.typeName}" /><i id="nameTag1"></i><i id="nameTag"></i></li>
    <li><label>&nbsp;</label>
...
$(function() {
  ...
  $("#typeName").blur(function() {
		$.ajax({
				type : "post",
				url : "ProductTypeServlet?method=isProductTypeExitUpdate",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
				...
				success : function(data) {
					...
					if(d=="0"){
						 $("#nameTag").text("");
						 $("#nameTag1").text("分类名称不能为空。").css({"color":"red","font-size":"12px"});
					}else if(d=="1") {
						 $("#nameTag1").text("分类名称已存在。").css({"color":"red","font-size":"12px"});
						  $("#nameTag").text("");
						 }else if(d=="2"){
						   $("#nameTag").text("√").css({"font-family":"Arial","color":"green","font-size":"12px"});
						    $("#nameTag1").text("");
						   }else if(getByteLen($("#name").val())>50){
								 $("#nameTag1").text("分类名称字数不能大于25。").css({"color":"red","font-size":"12px"});
								 $("#nameTag").text("");
							 }else{
