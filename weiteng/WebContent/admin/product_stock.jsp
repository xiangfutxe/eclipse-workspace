...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[4][10].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-云仓查询</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","ProductStockServlet?method=admin_list_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","ProductStockServlet?method=admin_list_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","ProductStockServlet?method=admin_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="ProductStockServlet?method=admin_list" method="post">
    <ul class="seachform">
 ...
 <li><label>产品编号</label><input name="productId" type="text" class="scinput" value="${productId}" /></li>
    ...
    <table class="tablelist">
       ...
       <td><a href="ProductStockServlet?method=transfers_add&id=${result.id}">调拨</a></td> 
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="ProductStockServlet?method=admin_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="ProductStockServlet?method=admin_list&productId=${productId}&userId=${userId}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount<=5}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ProductStockServlet?method=admin_list&productId=${productId}&userId=${userId}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         </c:forEach>
          </c:if>
           <c:if test="${pager.pageCount>5}">
            <!-- 首页 -->
             <c:if test="${pager.pageNo==1}"><li class="paginItem current" ><a href="javascript:;">1</a></li></c:if>
     	   	 <c:if test="${1!=pager.pageNo}"><li class="paginItem" ><a href="ProductStockServlet?method=admin_list&productId=${productId}&userId=${userId}&pageSizeStr=${pager.pageSize}&pageNoStr=1">1</a></li></c:if>
           ...
           <!-- 尾页 -->
            <c:if test="${pager.pageCount==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
        	 <c:if test="${pager.pageCount!=pager.pageNo}"><li class="paginItem" ><a href="ProductStockServlet?method=admin_list&productId=${productId}&userId=${userId}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
        ...
        <li class="paginItem"><a href="javascript:;"><span class="pagenxt"></span></a></li>
           ...
           <c:if test="${pager.pageCount-pager.pageNo>0}">
        <li class="paginItem"><a href="ProductStockServlet?method=admin_list&productId=${productId}&userId=${userId}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
