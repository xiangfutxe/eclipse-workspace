...
<body>
    ...
    <div class="formtitle"><span>基本信息</span></div>
     ...
     <form action="ProductTypeServlet?method=saveProductType" id="myform" method="post">
    <ul class="forminfo">
    <li><label>分类名称<b>*</b></label>
    <input type="hidden" name="token" id="token" value="<c:out value='${sessionScope.token}' escapeXml='true' default=''/>"/>
    <input name="typeName" id="typeName" type="text" class="dfinput" /><i id="nameTag1"></i><i id="nameTag"></i></li>
...
$(function() {
  ...
  $("#typeName").blur(function() {
		$.ajax({
				type : "post",
				url : "ProductTypeServlet?method=isProductTypeExit",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
  ...
  $("#remark").blur(function() {
		if($("#remark").val()=="")$("#remarkTag").text("分类描述不能为空！").css({"color":"red","font-size":"9px"});
		else $("#remarkTag").text("");
	});
