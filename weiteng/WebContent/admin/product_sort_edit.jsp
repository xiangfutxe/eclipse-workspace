...
out.println("</script>");
	  ...
	  String[][] ranks = StringUtil.getRankStr(admin.getRank());
			if(ranks[3][10].equals("1")||admin.getState()-1==0){
 %>
...
<body>
    ...
    <div class="formtitle"><span>基本信息</span></div>
     ...
     <form action="productSortServlet?method=update" id="myform" method="post">
      <input type="hidden" name="id" id="id" value="${productSort.id }"/> 
                 <input type="hidden" name="token" id="token" value="${sessionScope.token }"/> 
    <ul class="forminfo">
    <li><label>类型名称<b>*</b></label>
    <input name="name" id="name" type="text" class="dfinput" value="${productSort.name}" /><i id="nameTag1"></i><i id="nameTag"></i></li>
    <li><label>&nbsp;</label>
    <%if(ranks[3][12].equals("1")||admin.getState()-1==0){ %>
...
$(function() {
  ...
  $("#name").blur(function() {
		$.ajax({
				type : "post",
				url : "ProductSortServlet?method=isExitUpdate",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
					typeName : $("#name").val(),
					id : $("#id").val()
				...
				success : function(data) {
					...
					if(d=="0"){
						 $("#nameTag").text("");
						 $("#nameTag1").text("类型名称不能为空。").css({"color":"red","font-size":"12px"});
					}else if(d=="1") {
						 $("#nameTag1").text("类型名称已存在。").css({"color":"red","font-size":"12px"});
						  $("#nameTag").text("");
						 }else if(d=="2"){
						   $("#nameTag").text("√").css({"font-family":"Arial","color":"green","font-size":"12px"});
						    $("#nameTag1").text("");
						   }else if(getByteLen($("#name").val())>50){
								 $("#nameTag1").text("类型名称字数不能大于25。").css({"color":"red","font-size":"12px"});
								 $("#nameTag").text("");
							 }else{
