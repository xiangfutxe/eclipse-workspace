...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","ProductSortServlet?method=add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","ProductSortServlet?method=remove");
  $("#myform").submit();
});
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="seachform" action="ProductSortServlet?method=list" method="post">
    <ul class="seachform">
    	
    <li><label>类型名称</label><input name="typeName" type="text" class="scinput" value="${typeName}" /></li>
    ...
    <table class="tablelist">
      ...
      <th>类型名称</th>
        ...
        <td><a href="ProductSortServlet?method=edit&id=${result.id}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="ProductSortServlet?method=list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="ProductSortServlet?method=list&typeName=${typeName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ProductSortServlet?method=list&typeName=${typeName}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ProductSortServlet?method=list&typeName=${typeName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="ProductSortServlet?method=list&typeName=${typeName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
