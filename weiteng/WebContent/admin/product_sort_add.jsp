...
out.println("</script>");
	  ...
	  String[][] ranks = StringUtil.getRankStr(admin.getRank());
			if(ranks[3][11].equals("1")||admin.getState()-1==0){
 %>
...
<body>
    ...
    <div class="formtitle"><span>基本信息</span></div>
     ...
     <form action="ProductSortServlet?method=save" id="myform" method="post">
    <ul class="forminfo">
...
$(function() {
  ...
  $("#name").blur(function() {
		$.ajax({
				type : "post",
				url : "ProductSortServlet?method=isExit",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
				data : {//设置数据源
					typeName : $("#name").val()
  ...
  $("#remark").blur(function() {
		if($("#remark").val()=="")$("#remarkTag").text("类型描述不能为空！").css({"color":"red","font-size":"9px"});
		else $("#remarkTag").text("");
	});
