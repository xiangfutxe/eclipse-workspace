...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[1][1].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-会员查询</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","UserServlet?method=admin_list_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","UserServlet?method=admin_list_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","UserServlet?method=admin_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="UserServlet?method=admin_list" method="post">
    <ul class="seachform">
     
     
    <li><label>会员等级:</label>
     <select id="rankJoin" name="rankJoin" class="dfinput1">
    <option value="">--所有会员等级--</option>
    <option value="0"  <c:if test="${rankJoin=='0' }">selected</c:if>>--关注会员--</option>
	<option value="1"  <c:if test="${rankJoin=='1' }">selected</c:if>>--普通会员--</option>
    <option value="2" <c:if test="${rankJoin=='2' }">selected</c:if>>--天使会员--</option>
   <option value="3" <c:if test="${rankJoin=='3' }">selected</c:if>>--资深会员--</option>
   <option value="4"  <c:if test="${rankJoin=='4' }">selected</c:if>>--高级合伙人--</option>
    <option value="5" <c:if test="${rankJoin=='5' }">selected</c:if>>--城市合伙人--</option>
   <option value="6" <c:if test="${rankJoin=='6' }">selected</c:if>>--联创合伙人--</option>
   <option value="7"  <c:if test="${rankJoin=='7' }">selected</c:if>>--官方合伙人--</option>
   </select>
    </li>
  <li><label>会员信息</label><input name="userId" type="text" class="scinput" value="${userId}" /></li>
 <li><label>真实姓名</label><input name="userName" type="text" class="scinput" value="${userName}" /></li>
<li><label>真实昵称</label><input name="nickName" type="text" class="scinput" value="${nickName}" /></li>
</ul>
 <ul class="seachform">                      
<li><label>邀请人</label><input name="refereeId" type="text" class="scinput" value="${refereeId}" /></li>
     ...
     <li><input name="sub" id="search_sub" type="submit" class="scbtn" value="查询"/>
     &nbsp;<input name="exportExcel" id="exportExcel" type="submit" class="scbtn" value="导出EXCEL"/></li>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="UserServlet?method=admin_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&userName=${userName}&nickName=${nickName}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount<=5}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&userName=${userName}&nickName=${nickName}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         </c:forEach>
          </c:if>
           <c:if test="${pager.pageCount>5}">
            <!-- 首页 -->
             <c:if test="${pager.pageNo==1}"><li class="paginItem current" ><a href="javascript:;">1</a></li></c:if>
     	   	 <c:if test="${1!=pager.pageNo}"><li class="paginItem" ><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&userName=${userName}&nickName=${nickName}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=1">1</a></li></c:if>
           ...
           <!-- 尾页 -->
            <c:if test="${pager.pageCount==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
        	 <c:if test="${pager.pageCount!=pager.pageNo}"><li class="paginItem" ><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&userName=${userName}&nickName=${nickName}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
        ...
        <li class="paginItem"><a href="javascript:;"><span class="pagenxt"></span></a></li>
           ...
           <c:if test="${pager.pageCount-pager.pageNo>0}">
        <li class="paginItem"><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&userName=${userName}&nickName=${nickName}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
