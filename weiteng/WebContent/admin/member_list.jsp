...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
   $("#myform").attr("action","member_del.do");
  $("#myform").submit();
});
...
$(".remove_click").click(function(){
  ...
  $(".remove_tip").fadeOut(100);
   $("#myform").attr("action","member_remove.do");
  $("#myform").submit();
});
...
<body>
    ...
    <div class="tools">
    	<ul class="toolbar">
        <li  class="remove_click"><span><img src="images/t03.png" /></span>批量停用</li>
    ...
    <table class="tablelist">
        ...
        <c:forEach items="${pager.resultList}" var="member" varStatus="status">
        <tr>
        ...
        <c:if test="${member.rank==5}">钻石会员</c:if>
        </td>
          <td>${member.mobile}</td>
        <td><fmt:formatDate value="${member.entryTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
         <td>  <c:if test="${member.state==1}">正常</c:if><c:if test="${member.state==0}"><i>已删除</i></c:if></td>
        <td><a href="member_detail.do?id=${member.id}" class="tablelink">查看</a> 
        <c:if test="${member.state==1}">&nbsp;<a href="member_edit.do?id=${member.id}" class="tablelink">编辑</a>
        &nbsp;<a href="member_other.do?id=${member.id}" class="tablelink">其他</a>
        </c:if>
        </td>
        </tr>
        </c:forEach>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="member_list.do" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="member_list.do?pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="member_list.do?pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="member_list.do?pageSizeStr=${pager.pageSize()}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="member_list.do?pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
