...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","ParamServlet?method=add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","ParamServlet?method=remove");
  $("#myform").submit();
});
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="seachform" action="ParamServlet?method=list" method="post">
    <ul class="seachform">
    	
    <li><label>参数名称</label><input name="paramName" type="text" class="scinput" value="${paramName}" /></li>
    ...
    <table class="tablelist">
      ...
      <th>参数名称</th>
         ...
         <td>${result.amount_1}</td>
          <td>${result.amount_2}</td>
           <td>${result.amount_3}</td>
            <td>${result.amount_4}</td>
             <td>${result.amount_5}</td>
              <td>${result.amount_6}</td>
               <td>${result.amount_7}</td>
                <td>${result.amount_8}</td>
                 <td>${result.amount_9}</td>
                  <td>${result.amount_10}</td>
              <td>${result.unit}</td>
        ...
        <td><a href="ParamServlet?method=edit&id=${result.id}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="ParamServlet?method=list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="ParamServlet?method=list&paramName=${paramName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ParamServlet?method=list&paramName=${paramName}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ParamServlet?method=list&paramName=${paramName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="ParamServlet?method=list&paramName=${paramName}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
