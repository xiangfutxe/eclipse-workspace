...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[9][19].equals("1")||admin.getState()-1==0){
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>威腾后台-后台日志</title>
...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","AdminLogServlet?method=addJob");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","AdminLogServlet?method=removeAll");
  $("#myform").submit();
});
	...
	$('#search')
			.click(
					function() {
										...
										if (str == "") {
										$('#searchForm').attr("action","AdminLogServlet?method=list");
											$('#searchForm').submit();
										} else {
											alert(str);
											return false;
	 ...
	 $("#exportExcel").click(function(){
	  	  if (!confirm("确认提交导出Excel？")) {
				return false;
	       		...
	       		$("#searchForm").attr("action", "AdminLogServlet?method=exportExcel");
				$("#searchForm").submit();
	       		 return true;
			}
		 });
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="AdminLogServlet?method=list" method="post">
    <ul class="seachform">
    	
    <li><label>操作员</label><input name="adminName" type="text" class="scinput" value="${adminName}" /></li>
    <li><label>日志类型:</label>
     <select id="type" name="type" class="dfinput2">
       <option value="">全部</option>
      <option value="信息管理" <c:if test="${type=='信息管理'}">selected</c:if>>信息管理</option>
       <option value="会员管理" <c:if test="${type=='会员管理'}">selected</c:if>>会员管理</option>
        <option value="网络管理" <c:if test="${type=='网络管理'}">selected</c:if>>网络管理</option>
         <option value="产品管理" <c:if test="${type=='产品管理'}">selected</c:if>>产品管理</option>
          <option value="仓库管理" <c:if test="${type=='仓库管理'}">selected</c:if>>仓库管理</option>
           <option value="订单管理" <c:if test="${type=='订单管理'}">selected</c:if>>订单管理</option>
            <option value="奖金管理" <c:if test="${type=='奖金管理'}">selected</c:if>>奖金管理</option>
             <option value="财务管理" <c:if test="${type=='财务管理'}">selected</c:if>>财务管理</option>
             <option value="员工管理" <c:if test="${type=='员工管理'}">selected</c:if>>员工管理</option>
              <option value="其他设置" <c:if test="${type=='其他设置'}">selected</c:if>>其他设置</option>
                                    </select>
                                </li>
    ...
    <li><label>&nbsp;</label><input name="sub" id="search" type="submit" class="scbtn" value="查询"/>&nbsp;<input name="exportExcel" id="exportExcel" type="submit" class="scbtn" value="导出日志"/></li>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="AdminLogServlet?method=list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="AdminLogServlet?method=list?adminName=${adminName}&type=${type}&endTime=${endTime}&pageSizeStr=${pager.pageSize()}&pageNoStr=${pager.pageNo()-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="AdminLogServlet?method=list?adminName=${adminName}&type=${type}&endTime=${endTime}&pageSizeStr=${pager.pageSize()}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="AdminLogServlet?method=list?adminName=${adminName}&type=${type}&endTime=${endTime}&pageSizeStr=${pager.pageSize()}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="AdminLogServlet?method=list?adminName=${adminName}&type=${type}&endTime=${endTime}&pageSizeStr=${pager.pageSize()}&pageNoStr=${pager.pageNo()+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
