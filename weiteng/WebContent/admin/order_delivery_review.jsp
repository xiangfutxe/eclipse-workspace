...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_order_review_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","OrderDeliveryServlet?method=admin_order_review_del");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","OrderDeliveryServlet?method=admin_order_review");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="OrderDeliveryServlet?method=admin_order_review" method="post">
    <ul class="seachform">
	...
	<option value="1"  <c:if test="${orderType=='2' }">selected</c:if>>--商城购物--</option>
    <option value="2" <c:if test="${orderType=='3' }">selected</c:if>>--特卖购物--</option>
   <option value="3" <c:if test="${orderType=='4' }">selected</c:if>>--积分购物--</option>
   </select>
                                </li>
                                </ul>
          <ul class="seachform">                      
    ...
    <table class="tablelist">
        ...
        <td><a href="OrderDeliveryServlet?method=admin_order_detail_review&id=${result.orderId}" class="tablelink">详情</a> </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="OrderDeliveryServlet?method=admin_order_review" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_order_review&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo()-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_order_review&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="OrderDeliveryServlet?method=admin_order_review&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="OrderDeliveryServlet?method=admin_order_review&orderId=${orderId}&userId=${userId}&orderType=${orderType}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo()+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
