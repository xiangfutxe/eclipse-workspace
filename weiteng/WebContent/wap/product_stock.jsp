...
List<ProductStock> plist = (List<ProductStock>) request.getAttribute("product_stock");
double totalPrice =0;
...
<div class="fanhui_cou">
    ...
    <div class="fix_nav">
        <div class="nav_inner">
            ...
            <div class="tit">云仓库存</div>
   			<a class=" nav-right home-icon" href="UserServlet?method=index">首页</a>
            
        </div>
    </div>
</header>
<form action="OrderDeliveryServlet?method=stock_cart_confirm" method="post" id="myform" name="myform">
		            ...
		            <li class="list-group-item hproduct clearfix">
			                ...
			                <div class="p-info">
			                   <p class="p-title"><%=plist.get(i).getProductName()%></p>
		                    	...
		                    	<p class="p-origin">
			                       单价： <em class="price">¥<%=plist.get(i).getPrice()%></em>；当前库存：<%=plist.get(i).getNum()%>
			                   <input type="hidden" id="totalnum_<%=plist.get(i).getId()%>" value="<%=plist.get(i).getNum()%>">
					 ...
					 <li class="list-group-item clearfix">
		                <div class="pull-left mt5">
		                	...
		                	<em class="red productTotalPrice" id="price_<%=plist.get(i).getId()%>" >¥0</em>
		                </div>
						<div class="btn-group btn-group-sm control-num">
		                    ...
		                    <input type="tel" id="num_<%=plist.get(i).getId() %>" class="btn gary2 Amount" readonly="readonly" value="0" maxlength="4" itemkey="" prodId="663" skuId="1358">
...
<div class="fixed_inner">
    ...
    <div class="buy-btn-fix">
        <a href="javascript:void(0)"  id="submitShopCart" class="btn-cart">申请提货</a>
    </div>
</div>
</div>
</form>
...
function increate(index) {
	$.ajax({
					type : "post",
					url : "OrderDeliveryServlet?method=stock_add_ajax",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
					data : "tag="+index+"&totalNum="+$("#totalnum_"+index).val(),
...
function disDe(index) {
	...
	$.ajax({
					type : "post",
					url : "OrderDeliveryServlet?method=stock_sub_ajax",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
					data : "tag="+ index,
					dataType : "json",//设置需要返回的数据类型
					success : function(data) {
						var d = data;//将数据转换成json类型，可以把data用alert()输出出来看看到底是什么样的结构
