...
Integer num_int = (Integer)request.getAttribute("num");
int num=0;
if(num_int!=null) num = num_int;
...
<header class="header">
 ...
 <form action="OrderServlet?method=shop_list"  method="post" id="myform" name="myform">
  <input  type='hidden' name="tag" id="tag" value="1"/>
 
 <div class="content-list" id="outer">
	    	<div class="list-left" id="tab">
	    	<li <%if(StringUtil.notNull(idstr).equals("0")&&num==0){%> class="current"<%} %>>
	    	<a href="OrderServlet?method=shop_list&tag=1">全部</a></li>
	    	<%
	    	Collection coll_pt = (Collection) request.getAttribute("coll_pt");
	    	 ...
	    	 %>
	    		...
	    		<a href="OrderServlet?method=shop_list&id=<%=p.getId()%>&tag=1&num=1"><%=p.getName() %></a>
	    		</li>
	    	<%}} %>
	    	</div>
	    	...
	    	Collection coll = (Collection) request.getAttribute("coll");
	    	 ...
	    	 %>
			    	<li>
			    		<%if(num==0){ %>
		                	<a href="ProductServlet?method=product_detail&id=<%=p.getProductId()%>">
		                	<%}else{ %>
		                	<a href="OrderServlet?method=shop_confirm&id=<%=p.getProductId()%>&num=<%=num%>">
		                	<%} %>
			    		...
			    		<div class="shop-list-mid">
		                	...
		                	<a href="OrderServlet?method=shop_confirm&id=<%=p.getProductId()%>&num=<%=num%>">
		                	<%} %>
		                	<span class="f16"><%=p.getProductName() %></span></a></div>
		                	<div>规格:<%=p.getSpecification()%>；&nbsp;库存:<%=StringUtil.decimalFormat(p.getTotalNum())%>；</div>
		                	
		                	<div class="am-gallery-desc">单价：¥<%=StringUtil.decimalFormat(p.getPrice())%>；</div>
...
<footer class="footer">
  ...
  <div class="foot-right">
           <a href="OrderServlet?method=shop_cart"  id="submitShopCart" class=" btn-cart">进入购物车</a>
   </div>
   </div>
</footer>
</form>
