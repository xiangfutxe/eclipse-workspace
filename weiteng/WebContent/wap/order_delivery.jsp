...
<header class="header">
	<div class="fix_nav">
		<div class="nav_inner">
			...
			<div class="tit">
			<%if(state.equals("")){ %>配货列表
			<%}else if(state.equals("1")){ %>待确认<%}else if(state.equals("2")){ %>待发货
			<%}else if(state.equals("3")){ %>已发货<%}else if(state.equals("4")){ %>已签收
			<%} %>
			</div>
		</div>
	</div>
 ...
 Pager pager = (Pager)request.getAttribute("pager");
						if(pager!=null){
							...
							if(coll!=null){
									...
									while(it1.hasNext()){
									OrderDelivery p = (OrderDelivery)it1.next();
							%>
		<a href="OrderDeliveryServlet?method=order_detail&id=<%=p.getOrderId()%>">
				  <ul class="list-group">
		            ...
		            <span  class="pull-right font-14"><%=StringUtil.parseToString(p.getOrderTime(), DateUtil.yyyyMMddHHmmss)%>
			         ...
			         <br>
					       ...
					       <p>
					        ...
					        <%}else if(p.getState()==3){ %>已发货
					        <%}else if(p.getState()==4){ %>已签收
		            ...
		            <li class="list-group-item clearfix">
						...
						<%if(p.getState()==3){ %>
						 <a  href="OrderDeliveryServlet?method=order_confirm&id=<%=p.getOrderId() %>" class="btn1 btn-orders">
		                   <span class="glyphicon gary">确认收货</span></a>
		                   <%} %>
						 <a  href="OrderDeliveryServlet?method=order_express&id=<%=p.getOrderId() %>" class="btn1 btn-orders">
		                   <span class="glyphicon gary">物流跟踪</span></a>
		                    <a  href="OrderDeliveryServlet?method=order_detail&id=<%=p.getOrderId() %>" class="btn1 btn-orders">
...
<%if(pager.getPageNo()==1) {%> 首页&nbsp;|&nbsp;上一页&nbsp;|&nbsp;<%}else{%>
					<a
						href="OrderDeliveryServlet?method=orders&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a>&nbsp;|&nbsp;<a
						href="OrderDeliveryServlet?method=orders&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>&nbsp;|&nbsp;<%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a href="OrderDeliveryServlet?method=orders&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a>&nbsp;|&nbsp;<a
						href="OrderDeliveryServlet?method=orders&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
				<%}else{ %>下一页&nbsp;|&nbsp;尾页<%} %>
<%} %>
		</div>
		</div>
