...
<header class="header">
 ...
 <form action="ProductServlet?method=product_list"  method="post" id="myform" name="myform">
 ...
 <div class="content-list" id="outer">
	    	...
	    	<a href="ProductServlet?method=product_list&tag=1">全部</a></li>
	    	<%
	    	Collection coll_pt = (Collection) request.getAttribute("coll_pt");
	    	 ...
	    	 %>
	    		...
	    		<a href="ProductServlet?method=product_list&id=<%=p.getId()%>&tag=1"><%=p.getName() %></a>
	    		</li>
	    	<%}} %>
	    	</div>
	    	 ...
	    	 %>
			    	...
			    	<!--  	<img src="../upload/<%=p.getImageUrl() %>" style="width:80px" class="list-pic" />-->
			    		<div class="shop-list-mid">
		                	...
		                	<div><a href="ProductServlet?method=detail&id=<%=p.getProductId()%>"><%=p.getProductName() %></a></div>
		                	<div><a href="ProductServlet?method=detail&id=<%=p.getProductId()%>">规格:<%=p.getSpecification()%>；&nbsp;库存:<%=StringUtil.decimalFormat(p.getTotalNum())%>；</a></div>
