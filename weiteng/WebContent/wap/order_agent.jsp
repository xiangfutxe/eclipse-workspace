		            ...
		            <li class="list-group-item clearfix">
						...
						<div class="btn-group btn-group-sm control-num">
		                   ...
		                   <%if(p.getOrderType()==1&&p.getState()==1) {%>
		                     <a onclick='javascript:if(!confirm("确认进行确认已收款？")) return false;' href="OrderServlet?method=order_confirm&id=<%=p.getId() %>&orderId=<%=p.getOrderId() %>" class="btn1 btn-orders"><span class="glyphicon gary">确认收款</span></a>
...
<%if(pager.getPageNo()==1) {%> 首页&nbsp;|&nbsp;上一页&nbsp;|&nbsp;<%}else{%>
					<a
						href="OrderServlet?method=order_agent_list&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a>&nbsp;|&nbsp;<a
						href="OrderServlet?method=order_agent_list&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>&nbsp;|&nbsp;<%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a href="OrderServlet?method=order_agent_list&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a>&nbsp;|&nbsp;<a
						href="OrderServlet?method=order_agent_list&state=<%=StringUtil.notNull(state)%>&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
				<%}else{ %>下一页&nbsp;|&nbsp;尾页<%} %>
<%} %>
		</div>
		</div>
