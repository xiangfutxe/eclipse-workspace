...
<header class="header">
	<div class="fix_nav">
		<div class="nav_inner">
			...
			<div class="tit">特卖专区</div>
			<a class=" nav-right home-icon" href="UserServlet?method=index">首页</a>
		</div>
	</div>
</header>

 <form action="OrderServlet?method=shop_cash_list"  method="post" id="myform" name="myform">
 ...
 <div class="content-list" id="outer">
	    	...
	    	<a href="OrderServlet?method=shop_cash_list&tag=1">全部</a></li>
	    	</div>
	    	<div class="list-right" id="content">
	    		<ul class="list-pro">
	    			<li>
			     <img src="images/temai.jpeg" style="width:100%" class="list-pic" />
			     </li>
	    		<%
	    	 ...
	    	 %>
			     ...
			     <img src="../upload/product/<%=p.getImageUrl() %>" style="width:80px" class="list-pic" />
			    		<div class="shop-list-mid">
		                	...
		                	<div><a href="ProductServlet?method=product_cash_detail&id=<%=p.getProductId()%>"><span class="f16"><%=p.getProductName() %></span></a></div>
		                	<div><a href="ProductServlet?method=product_cash_detail&id=<%=p.getProductId()%>">规格:<%=p.getSpecification()%>；&nbsp;库存:<%=StringUtil.decimalFormat(p.getTotalNum())%>；</a></div>
...
<footer class="footer">
  ...
  <div class="foot-right">
   ...
   <a href="OrderServlet?method=shop_cash_cart"  id="submitShopCart" class="btn-cart">进入购物车</a>
