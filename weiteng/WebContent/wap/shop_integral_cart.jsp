...
<div class="fanhui_cou">
    ...
    <div class="fix_nav">
        <div class="nav_inner">
            <a class="nav-left back-icon" href="OrderServlet?method=shop_integral_list&tag=1">返回</a>
...
<form action="OrderServlet?method=shop_integral_confirm" method="post" id="myform" name="myform">
...
function increate(index) {
	$.ajax({
					type : "post",
					url : "OrderServlet?method=increate_ajax_integral",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
					data : "tag="+index,
...
function disDe(index) {
	...
	$.ajax({
					type : "post",
					url : "OrderServlet?method=sub_ajax_integral",//需要用来处理ajax请求的action,excuteAjax为处理的方法名，JsonAction为action名
					data : "tag="+ index,
					dataType : "json",//设置需要返回的数据类型
					success : function(data) {
						var d = data;//将数据转换成json类型，可以把data用alert()输出出来看看到底是什么样的结构
