	...
	public List<WithDrew> findByList(WithDrew with_drew){
 	...
 	public String apply_save(double amount,User user){
		...
		String applyId = "W"+StringUtil.parseToString(date, DateUtil.ymdhms)+String.valueOf(cs.getRandom(10,99));
		 ...
		 if(p!=null){
			 User user1 = userMapper.selectByUserIdForUpdate(user.getUserId());
			 if(user1!=null){
			 ...
			 ad.setUid(user1.getId());
	    		...
	    		ad.setTradeType(Constants.MONEY_DETAIL_YYPE_12);
	    		ad.setSummary("奖金提现");
	    		ad.setPayType(2);
    			...
    			if(ad.getBalance()>=0){
					 ...
					 update_user.setVersion(user1.getVersion());
...
public String apply_no(String adminName,String applyId,Timestamp date){
 		...
 		try {
 			...
 			if(wd!=null){
 				if(wd.getState()==1){
			 User user1 = userMapper.selectByUserIdForUpdate(wd.getUserId());
			 if(user1!=null){
