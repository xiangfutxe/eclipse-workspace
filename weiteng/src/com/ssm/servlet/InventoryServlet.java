...
public class InventoryServlet extends HttpServlet {
	...
	String inventoryName = StringUtil.notNull(request.getParameter("name"));
	...
	int pageSize = 20;
		...
		try {
			...
			String request_dispatcher ="inventory_message.jsp";
			if (!message.equals("")) {
				request_dispatcher ="inventory_message.jsp";
				request.setAttribute("message", message);
			}else if (!error.equals("")) {
				if(lt==1) request_dispatcher ="error_login.jsp";
				else request_dispatcher ="inventory_error.jsp";
				request.setAttribute("error", error);
			}else {
				request_dispatcher ="inventory.jsp";
			}
	...
	Admin admin = (Admin) request.getSession().getAttribute(Constants.ADMIN_SESSION);
		...
		try {
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_edit.jsp";
			}
	...
	Admin admin = (Admin) request.getSession().getAttribute(Constants.ADMIN_SESSION);
		...
		try {
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_add.jsp";
			}
		...
		try {
			if(admin!=null){
				if (StringUtil.notNull(token_old).equals(token)) {
					...
					if(result==1) message="仓库信息保存成功。";
					else if(result==2) error="仓库信息更新异常。";
					else error="仓库信息未发生变化。";
						inventory=null;
					}else {
						error="仓库名、负责人、所在地和联系电话不能为空，请重新录入！";
					}
				}else{
					error = "请勿重复提交数据。";
				}
		...
		try {
			if(admin!=null){
				...
				if(!(StringUtil.notNull(inventoryName).equals("")||StringUtil.notNull(linkman).equals("")||
					...
					inventory.setEndTime(date);
					int result = inventoryDao.saveInventory(inventory);
					if(result==1) message="仓库信息保存成功。";
					else if(result==2) error="该仓库名存在。";
					else error="仓库信息保存失败。";
						inventory=null;
				}else {
					error="仓库名、负责人、所在地和联系电话不能为空，请重新录入！";
	...
	int pageSize = 20;
		...
		try {
			...
			String request_dispatcher ="inventory_apply_in_message.jsp";
			if (!message.equals("")) {
				request_dispatcher ="inventory_apply_in_message.jsp";
				request.setAttribute("message", message);
			}else if (!error.equals("")) {
				if(lt==1) request_dispatcher ="error_login.jsp";
				else request_dispatcher ="inventory_apply_in_error.jsp";
				request.setAttribute("error", error);
			}else {
				request_dispatcher ="inventory_apply_in.jsp";
			}
		...
		try {
			if(admin!=null){
				...
				if(!state.equals(""))
					iay.setState(Integer.valueOf(state));
				if(iay.getState()-1!=0) {
					iay.setReviewerId(admin.getAdminName());
				}
				if(!startTimeStr.equals("")){
					Timestamp startTime = new Timestamp(StringUtil.parseToDate(startTimeStr+" 00:00:00", DateUtil.yyyyMMddHHmmss).getTime());
					iay.setStartTime(startTime);
			...
			String request_dispatcher ="inventory_apply_in_review_message.jsp";
			if (!message.equals("")) {
				request_dispatcher ="inventory_apply_in_review_message.jsp";
				request.setAttribute("message", message);
			}else if (!error.equals("")) {
				if(lt==1) request_dispatcher ="error_login.jsp";
				else request_dispatcher ="inventory_apply_in_review_error.jsp";
				request.setAttribute("error", error);
			}else {
				request_dispatcher ="inventory_apply_in_review.jsp";
			}
	...
	public void apply_in_add(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_in_add.jsp";
			}
		...
		try {
			if(admin!=null){
				if(StringUtil.notNull(token_old).equals(token)){
					...
					InventoryDao inventoryDao = new InventoryDao();
					Inventory inventory= inventoryDao.findById(Integer.valueOf(inventoryId));
					if(inventory!=null) {
					InventoryApplyDao inventoryApplyDao = new InventoryApplyDao();
					 Timestamp date = new Timestamp(new Date().getTime());
						String applyId = "I"+StringUtil.parseToString(date, DateUtil.ymdhms)+String.valueOf(cs.getRandom(10,99));

					message = inventoryApplyDao.saveApply(admin,applyId,inventory,1,type,ids,pids,names,prices,specifications,nums,remark,date);
	...
	public void apply_in_detail(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_in_detail.jsp";
			}
	...
	public void apply_in_review_detail(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_in_review_detail.jsp";
			}
	...
	int pageSize = 20;
		...
		try {
			...
			String request_dispatcher ="inventory_apply_out_message.jsp";
			if (!message.equals("")) {
				request_dispatcher ="inventory_apply_out_message.jsp";
				request.setAttribute("message", message);
			}else if (!error.equals("")) {
				if(lt==1) request_dispatcher ="error_login.jsp";
				else request_dispatcher ="inventory_apply_out_error.jsp";
				request.setAttribute("error", error);
			}else {
				request_dispatcher ="inventory_apply_out.jsp";
			}
	...
	int pageSize = 20;
		...
		try {
			if(admin!=null){
				...
				if(!state.equals(""))
					iay.setState(Integer.valueOf(state));
				if(iay.getState()-1!=0) iay.setReviewerId(admin.getAdminName());
				if(!startTimeStr.equals("")){
					Timestamp startTime = new Timestamp(StringUtil.parseToDate(startTimeStr+" 00:00:00", DateUtil.yyyyMMddHHmmss).getTime());
					iay.setStartTime(startTime);
				}
			...
			String request_dispatcher ="inventory_apply_out_review_message.jsp";
			if (!message.equals("")) {
				request_dispatcher ="inventory_apply_out_review_message.jsp";
				request.setAttribute("message", message);
			}else if (!error.equals("")) {
				if(lt==1) request_dispatcher ="error_login.jsp";
				else request_dispatcher ="inventory_apply_out_review_error.jsp";
				request.setAttribute("error", error);
			}else {
				request_dispatcher ="inventory_apply_out_review.jsp";
			}
	...
	public void apply_out_add(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_out_add.jsp";
			}
		...
		try {
			if(admin!=null){
				if(StringUtil.notNull(token_old).equals(token)){
					...
					message = inventoryApplyDao.saveApply(admin,applyId,inventory,2,type,ids,pids,names,prices,specifications,nums,remark,date);
					if(message.equals("yes")){
						message="出库申请已提交，申请编号为："+applyId+"。";
					}else error= "未查询到需要出库的商品信息！";
					}else error= "未查询到对应仓库的信息！";
				}else{
					error="请勿重复提交数据，请在仓库列表中查看是否保存成功！";
			}
			}else{
				error= "管理员用户未登陆，请重新登陆！";
				lt++;
	...
	public void apply_out_detail(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_out_detail.jsp";
			}
	...
	public void apply_out_review_detail(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		if(admin!=null){
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="inventory_apply_out_review_detail.jsp";
			}
