...
public class OrderServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		}else if(method.equals("pay_orders_cash")){
			pay_orders_cash(request, response);
		...
		try {
			if(user!=null){
				Timestamp date = new Timestamp(new Date().getTime());
					...
					if(message.equals("yes"))
						message="订单"+orderId+"已确认支付，如是商城购物，请到云仓中申请提货。";
			}else{
				message = "用户未登陆，请重新登陆。";
				lt++;
			}
	...
	if(plist!=null&&plist.size()>0) {
		...
		Timestamp date = new Timestamp(new Date().getTime());
		String orderId = StringUtil.parseToString(date, DateUtil.ymdhms)+cs.getRandom(1, 9);
		Order orders = orderDao.saveTeOrders(user.getUserId(),adrId, orderId, payType, plist, date);
		if(orders!=null&&orders.getMsg().equals("yes")) {
	...
	public void pay_orders(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		...
		try {
			...
			if(orders!=null) {
				...
				paraMap.put("total_fee",StringUtil.decimalFormat3(ArithUtil.add(orders.getPrice(),orders.getFee())*100));  
	...
	public void pay_orders_cash(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		WeixinUserInfo user  = (WeixinUserInfo) request.getSession().getAttribute(Constants.USER_SESSION);
		try {
			//String getopenid_url = "https://api.weixin.qq.com/sns/oauth2/access_token";
				//String  param="appid="+SignUtil.APPID+"&secret="+SignUtil.APPSECRET+"&code="+code+"&grant_type=authorization_code";
