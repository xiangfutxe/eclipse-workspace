...
public class CustomService implements ICustomService {
	 ...
	 public AccountDetail returnAccountDetail(User user,double amount,double balance,int payType,String tradeType,String summary,Timestamp date)
	 {
		 ...
		 acde.setUid(user.getId());
		 acde.setUserId(user.getUserId());
		 acde.setUserName(user.getUserName());
		 acde.setAmount(amount);
		 acde.setBalance(balance);
		 acde.setPayType(payType);
		 acde.setTradeType(tradeType);
		 acde.setSummary(summary);
		 acde.setEntryTime(date);
		 return acde;
	 }
