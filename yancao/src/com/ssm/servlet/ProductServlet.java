	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="product.jsp";
			}
	...
	int lt=0;
		try {
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="product_add.jsp";
			}
	...
	int type=1;
		try {
			if(admin!=null){
				...
				if(!typestr.equals(""))type = Integer.valueOf(typestr);
				...
				pt.setProductSort(productSort);
				pt.setFeatures(features);
				pt.setCash(Double.valueOf(cash));
				pt.setCashNum(Integer.valueOf(cashNum));
				pt.setIntegral(Double.valueOf(integral));
				pt.setType(type);
				pt.setPrice(Double.valueOf(price));
				pt.setPrice1(Double.valueOf(price1));
				pt.setIsHide(Integer.valueOf(isHide));
				pt.setState(1);
				pt.setEntryTime(date);
				pt.setEndTime(date);
				if(type==1) {
					int result = ptDao.saveProduct(pt);
					if(result==0) message=pt.getProductId()+"信息保存成功，请在产品查询中继续上传图片或设置会员价。";
					else if(result==1) error="该产品编号已经存在。";
					else if(result==2) error="数据操作异常，请核对。";
					pt=null;
				}else {
					ptDao = new ProductDao();
					Product sel_pro = new Product();
					sel_pro.setType(1);
					List<Product> plist = ptDao.findByProduct(sel_pro);
					request.setAttribute("coll", plist);
					request.getSession().setAttribute("session_product", pt);
				}
			...
			if (!message.equals("")) {
				...
				request_dispatcher ="product_select.jsp";
			}
			RequestDispatcher dispatcher = request
					.getRequestDispatcher(request_dispatcher);
			dispatcher.forward(request, response);
