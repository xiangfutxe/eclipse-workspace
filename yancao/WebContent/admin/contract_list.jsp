...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","ContractServlet?method=admin_list_add");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","ContractServlet?method=admin_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="ContractServlet?method=admin_list" method="post">
    <ul class="seachform">
     
  <li><label>合同编号</label><input name="contractId" type="text" class="scinput" value="${contractId}" /></li>
 <li><label>合同名称</label><input name="contractName" type="text" class="scinput" value="${contractName}" /></li>
<li><label>承办人</label><input name="adminName" type="text" class="scinput" value="${adminName}" /></li>
  <li><label>&nbsp;</label><input name="sub" id="search_sub" type="submit" class="scbtn" value="查询"/></li>
    ...
    <table class="tablelist">
        ...
        <td>${result.contractName}</td>
         <td>${result.code}</td>
          ...
          <td>
          <c:if test="${result.level==1}">保密</c:if>
          <c:if test="${result.level==2}">可公开</c:if>
         </td>
           <td>
          <c:if test="${result.state==1}">在馆</c:if>
          <c:if test="${result.state==2}">已借阅</c:if>
         </td>
        <td>
      
        <a href="ContractServlet?method=pdf&id=${result.contractId}" class="tablelink">点击查看</a>
       
        <a href="ContractServlet?method=pdf_edit&id=${result.contractId}" class="tablelink">上传PDF</a>
          <a href="ContractServlet?method=edit&id=${result.contractId}" class="tablelink">修改</a>
         <a  href="#" class="del_click tablelink">删除</a>
        </td>
        </tr>
        </c:forEach>
        </c:if>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="ContractServlet?method=admin_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="ContractServlet?method=admin_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount<=5}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="ContractServlet?method=admin_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         </c:forEach>
          </c:if>
           <c:if test="${pager.pageCount>5}">
            <!-- 首页 -->
             <c:if test="${pager.pageNo==1}"><li class="paginItem current" ><a href="javascript:;">1</a></li></c:if>
     	   	 <c:if test="${1!=pager.pageNo}"><li class="paginItem" ><a href="ContractServlet?method=admin_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=1">1</a></li></c:if>
           ...
           <!-- 尾页 -->
            <c:if test="${pager.pageCount==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
        	 <c:if test="${pager.pageCount!=pager.pageNo}"><li class="paginItem" ><a href="ContractServlet?method=admin_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
        ...
        <li class="paginItem"><a href="javascript:;"><span class="pagenxt"></span></a></li>
           ...
           <c:if test="${pager.pageCount-pager.pageNo>0}">
        <li class="paginItem"><a href="ContractServlet?method=admin_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
	...
	$(".del_click").click(function(){
	  ...
	  $(".del_tip").fadeOut(100);
	  $("#myform").attr("action","ContractServlet?method=del");
	  $("#myform").submit();
	  return true;
	});
