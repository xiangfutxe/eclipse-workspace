...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
   $("#myform").attr("action","NewsServlet?method=delAll");
  $("#myform").submit();
});
...
$(".remove_click").click(function(){
  ...
  $(".add_click").click(function(){
	  ...
	  $(".add_tip").fadeOut(100);
	   $("#myform").attr("action","NewsServlet?method=add");
	  $("#myform").submit();
	});
 ...
 <li class="add_click"><span><img src="images/t01.png" /></span>添加</li>
    ...
    <table class="tablelist">
        ...
        <td><fmt:formatDate value="${news.entryTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
         <td>  <c:if test="${news.state==1}">正常</c:if><c:if test="${news.state==0}"><i>已下线</i></c:if></td>
        <td><a href="NewsServlet?method=admin_news_detail&id=${news.id}" class="tablelink">查看</a> <c:if test="${news.state==1}"> <a href="NewsServlet?method=edit&id=${news.id}" class="tablelink">编辑</a></c:if>
        <c:if test="${news.state==1}"><a href="NewsServlet?method=admin_news_del&id=${news.id}" class="tablelink">下线</a></c:if><c:if test="${news.state==0}">
<a href="NewsServlet?method=admin_news_up&id=${news.id}" class="tablelink">上线</a></c:if>
        </td>
        </tr>
        </c:forEach>
        </c:if>    
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="NewsServlet?method=admin_news_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="NewsServlet?method=admin_news_list&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="NewsServlet?method=admin_news_list&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="NewsServlet?method=admin_news_list&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="NewsServlet?method=admin_news_list&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
