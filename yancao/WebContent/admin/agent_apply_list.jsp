...
if (admin == null) {
 ...
 String[][] ranks = StringUtil.getRankStr(admin.getRank());
	if(ranks[1][15].equals("1")||admin.getState()-1==0){
%>
...
$(document).ready(function(){
  ...
  $(".sure").click(function(){
  $(".tip").fadeOut(100);
  $("#myform").attr("action","AgentApplyServlet?method=admin_review_yes");
  $("#myform").submit();
});
...
$("#no").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","AgentApplyServlet?method=admin_review_no");
  $("#myform").submit();
});
  ...
  $("#ckAll").prop("checked" , $ids.length == $subs.filter(":checked").length ? true :false);
	...
	.click(
			function() {
								...
								if (str == "") {
								$('#searchForm').attr("action","AgentApplyServlet?method=admin_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="AgentApplyServlet?method=admin_list" method="post">
    <ul class="seachform">
  ...
  <li><label>会员信息</label><input name="userId" type="text" class="scinput" value="${userId}" /></li>
    ...
    <table class="tablelist">
        ...
        <td>${result.userId}</td>
         <td>${result.userName}</td>
          <td>${result.mobile}</td>
           <td>${result.weixin}</td>
          <td>
          <c:if test="${result.type==1}">市级</c:if>
          <c:if test="${result.type==2}">省级</c:if>
          <c:if test="${result.type==3}">全国</c:if>
         </td>
         <td>${result.province}${result.city}${result.area}</td>
        ...
        <% if(ranks[1][17].equals("1")||admin.getState()-1==0){%>
        <a href="AgentApplyServlet?method=admin_review_yes&id=${result.id}" class="tablelink" onclick="javascript:if(!confirm('是否确认通过该申请？')) return true;">通过</a>&nbsp;
         <a href="AgentApplyServlet?method=admin_review_no&id=${result.id}" class="tablelink" onclick="javascript:if(!confirm('是否确认拒绝该申请？')) return true;">拒绝</a>
        <%} %>
        </c:if>
         </td>
    ...
    <div class="pagin">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount>3}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         
         </c:forEach>
         <li class="paginItem more"><a href="javascript:;">...</a></li>
            <c:if test="${p==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
       ...
       <c:if test="${pager.pageCount<4}">
        ...
        <li class="paginItem"><a href="UserServlet?method=admin_list&refereeId=${refereeId}&userId=${userId}&state=${state}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
