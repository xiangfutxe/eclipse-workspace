...
$(document).ready(function(){
 ...
 $(".add_click").click(function(){
  ...
  $(".add_tip").fadeOut(100);
  $("#myform").attr("action","BorrowApplyServlet?method=admin_confirm_list_add");
  $("#myform").submit();
});
...
$(".del_click").click(function(){
  ...
  $(".del_tip").fadeOut(100);
  $("#myform").attr("action","BorrowApplyServlet?method=admin_confirm_list_del");
  $("#myform").submit();
});
	...
	.click(
			function() {
							...
							var startTime = $("#startTime")
								...
								if (str == "") {
								$('#searchForm').attr("action","BorrowApplyServlet?method=admin_confirm_list");
									$('#searchForm').submit();
								} else {
									alert(str);
									return false;
...
<body>
    ...
    <div class="rightinfo">
    	...
    	<form  id ="searchForm" action="BorrowApplyServlet?method=admin_confirm_list" method="post">
    <ul class="seachform">
 ...
 <li><label>合同名称</label><input name="contractName" type="text" class="scinput" value="${contractName}" /></li>
    ...
    <table class="tablelist">
        ...
        <td>${result.contractName}</td>
         <td>${result.code}</td>
          <td>${result.applyName}</td>
           <td><fmt:formatDate value="${result.entryTime}" pattern="yyyy-MM-dd"/></td>
            <td>${result.reviewName}</td>
         <td><fmt:formatDate value="${result.confirmTime}" pattern="yyyy-MM-dd"/></td>
           <td>${result.confirmName}</td>
          ...
          <td>
           <c:if test="${result.state==0}">审批不通过</c:if>
          <c:if test="${result.state==1}">待审批</c:if>
          <c:if test="${result.state==2}">审批通过</c:if>
          <c:if test="${result.state==3}">已借出</c:if>
           <c:if test="${result.state==4}">已归还</c:if>
         </td>
        <td>
      
         <c:if test="${result.state==2}">
        <a href="BorrowApplyServlet?method=admin_confirm_yes&id=${result.id}" class="tablelink">确认借出</a>
        </c:if>
        </td>
        </tr>
        </c:forEach>
    ...
    <div class="pagin">
    	...
    	<div class="message">
    	<form  name="pageSizeForm"  action="BorrowApplyServlet?method=admin_confirm_list" method="post">
       ...
       <c:if test="${pager.pageNo>1}">
        <li class="paginItem"><a href="BorrowApplyServlet?method=admin_confirm_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo-1}"><span class="pagepre"></span></a></li>
        </c:if>
         <c:if test="${pager.pageNo<=1}">
        ...
        <c:if test="${pager.pageCount<=5}">
         ...
         <c:if test="${p!=pager.pageNo}"><li class="paginItem" ><a href="BorrowApplyServlet?method=admin_confirm_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${p}">${p}</a></li></c:if>
         </c:forEach>
          </c:if>
           <c:if test="${pager.pageCount>5}">
            <!-- 首页 -->
             <c:if test="${pager.pageNo==1}"><li class="paginItem current" ><a href="javascript:;">1</a></li></c:if>
     	   	 <c:if test="${1!=pager.pageNo}"><li class="paginItem" ><a href="BorrowApplyServlet?method=admin_confirm_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=1">1</a></li></c:if>
           ...
           <!-- 尾页 -->
            <c:if test="${pager.pageCount==pager.pageNo}"><li class="paginItem current" ><a href="javascript:;">${pager.pageCount}</a></li></c:if>
        	 <c:if test="${pager.pageCount!=pager.pageNo}"><li class="paginItem" ><a href="BorrowApplyServlet?method=admin_confirm_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageCount}">${pager.pageCount}</a></li></c:if>
        ...
        <li class="paginItem"><a href="javascript:;"><span class="pagenxt"></span></a></li>
           ...
           <c:if test="${pager.pageCount-pager.pageNo>0}">
        <li class="paginItem"><a href="BorrowApplyServlet?method=admin_confirm_list&contractId=${contractId}&contractName=${contractName}&adminName=${adminName}&rankJoin=${rankJoin}&startTime=${startTime}&endTime=${endTime}&pageSizeStr=${pager.pageSize}&pageNoStr=${pager.pageNo+1}"><span class="pagenxt"></span></a></li>
        </c:if>
        </ul>
    </div>
    </c:if>
