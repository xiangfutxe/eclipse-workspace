	...
	<div class="second">   
        ...
        <jsp:include page="second.jsp"/>
            <div class="scd_mr">
            	...
            	<div class="ht_table">
            	<h2>当前位置：会员注册->免费注册-><a href="UserServlet.do?method=user_add_branch">分会选择</a></h2>
            	</div>
            	<div class="ht_table">
            	<ul>
            	 <li class="h2"><img alt="" src="images/703.jpg"></li>
         ...
         <span>分会编号</span>
							...
							%>
							<li  class="h1">
								<a  class="link-update" href="UserServlet.do?method=user_add&id=<%=as.getBranchId()%>"><%=StringUtil.notNull(as.getBranchName())%></a>
							
							<span>
								<a class="link-update"
									href="UserServlet.do?method=user_add&id=<%=as.getBranchId()%>"><%=StringUtil.notNull(as.getBranchId())%></a></span>
								</li>
							<%
								}
											}
						...
						<li>&nbsp;</li>
						 ...
						 <% if(pager!=null){ %>
						 <form name="pageNoForm"  action="UserServlet.do?method=user_add_branch" method="post">
						<input type="hidden" name="pageSizeStr" value="<%=pager.getPageSize()%>"/>
					第<font color="red"><%=pager.getPageNo()%>
					</font>页&nbsp; 共<font color="red"><%=pager.getPageCount()%></font>页共<font color="red"><%=pager.getRowCount()%></font>条记录&nbsp;
					<%if(pager.getPageNo()==1) {%> 首页 | 上一页  |<%}else{%>
					<a
						href="UserServlet.do?method=user_add_branch&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1">首页</a> | <a
						href="UserServlet.do?method=user_add_branch&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="UserServlet.do?method=user_add_branch&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>">
                    下一页</a> | <a
						href="UserServlet.do?method=user_add_branch&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>">尾页</a>
