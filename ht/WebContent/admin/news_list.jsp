		...
		<div class="main-wrap">
			...
			<div class="search-wrap">
				<div class="search-content">
					<form action="admin/NewsServlet.do?method=admin_news_list"
						id="searchForm" method="post">
						<table class="search-tab">
							<tr>

								<th width="120px">新闻栏目：</th>
								<td><select name="sort" id="sort" class="common-text">
										<option value="">--全部--</option>
										<option value="1"
											<%if (StringUtil.notNull(sort).equals("通知公告")) {%> selected
											<%}%>>--最新资讯--</option>
										<option value="2"
											<%if (StringUtil.notNull(sort).equals("公司动态")) {%> selected
											<%}%>>--分会动态--</option>
								</select></td>
								<th width="80">发布日期:</th>
				...
				<form name="delform" id="delform" method="post"
					...
					<div class="result-title">
						<div class="result-list">
							<a href="admin/NewsServlet.do?method=admin_news_add"><i
						...
						<table class="result-tab" width="100%">
							...
							<tr>
								...
								<td>
									<%
										if (as.getSort() == 1) {
									%>最新资讯<%
										} else if (as.getSort() == 2) {
									%>分会动态<%
										} else if (as.getSort() == 3) {
									%>全球活动<%
										}
									%>
								</td>
								...
								<td><a class="link-update"
									...
									onClick="javascript:if (!confirm('确认要将此新闻上线，上线后会员可以查看此新闻？'))return false;">上线</a>
									<%
										} else {
									%> <a class="link-del"
									href='admin/NewsServlet.do?method=admin_news_down&id=<%=as.getId()%>'
									onClick="javascript:if (!confirm('确认要将此新闻下线，下线后会员将无法查看？'))return false;">下线</a>|<a class="link-del"
									href='admin/NewsServlet.do?method=admin_news_del&id=<%=as.getId()%>'
									onClick="javascript:if (!confirm('确认要彻底删除此新闻，删除后将无法恢复？'))return false;">删除</a> <%}%></td>
							</tr>
							<%
								}
											}
							...
							%><tr>
								<td colspan="10">暂无新闻，请点击<a
									href="admin/NewsServlet.do?method=admin_news_add">添加新闻。</a>
								</td>
							</tr>
							<%
								}
				...
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm"
							action="admin/NewsServlet.do?method=admin_news_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm"
							...
							%>
							<a
								href="admin/NewsServlet.do?method=admin_news_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>&sort=<%=StringUtil.notNull(sort)%>">首页</a>
							| <a
