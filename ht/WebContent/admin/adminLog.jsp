...
String adminName = (String)request.getAttribute("adminName");
	...
	String[][] rankstr = StringUtil.getRankStr(admin.getRank());
		if(rankstr[9][5].equals("1")||admin.getState().equals("1")){
%>
		...
		$('#search')
				.click(
						function() {
											...
											if (str == "") {
											$('#searchForm').attr("action","admin/LogServlet.do?method=admin_log_list");
												$('#searchForm').submit();
											} else {
												alert(str);
												return false;
		...
		$('#delAll').click(
						function() {
										if (!confirm("确认要批量删除管理员工作日志？")) {	
											...
											if (str == "") {
											$('#searchForm').attr("action","admin/LogServlet.do?method=admin_log_delAll");
												$('#searchForm').submit();
												return true;
											} else {
												alert(str);
...
<body>
 ...
 <td><%=StringUtil.notNull(log.getContents()) %>
  ...
  <td><a href="admin/LogServlet.do?method=admin_log_del&id=<%=log.getId()%>">删除</a></td>
 </tr>
	 	<%}}
			 ...
			 <% if(pager!=null){ %>
				<div class="list-page">
					<div class="list-page-left">
						<form name="pageSizeForm" action="admin/LogServlet.do?method=admin_log_list"
							method="post">
					...
					<div class="list-page-right">
						<form name="pageNoForm" action="admin/LogServlet.do?method=admin_log_list"
							method="post">
					...
					<a
						href="admin/LogServlet.do?method=admin_log_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=1&adminName=<%=StringUtil.notNull(adminName)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">首页</a> | <a
						href="admin/LogServlet.do?method=admin_log_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()-1%>&adminName=<%=StringUtil.notNull(adminName)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">上一页</a>  | <%} %>
                 <%if(pager.getPageNo()!=pager.getPageCount()) {%>
					<a
						href="admin/LogServlet.do?method=admin_log_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageNo()+1%>
						&adminName=<%=StringUtil.notNull(adminName)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">
                    下一页</a> | <a
						href="admin/LogServlet.do?method=admin_log_list&pageSizeStr=<%=pager.getPageSize()%>&pageNoStr=<%=pager.getPageCount()%>&adminName=<%=StringUtil.notNull(adminName)%>&startTime=<%=StringUtil.notNull(startTime)%>&endTime=<%=StringUtil.notNull(endTime)%>">尾页</a>
