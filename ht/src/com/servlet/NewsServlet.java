...
public class NewsServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException, ServletException{
	...
	else if(method.equals("news_branch_list")){
		news_branch_list(request,response);
	...
	}else if(method.equals("news_branch_detail")){
		news_branch_detail(request,response);
		...
		}else if(method.equals("admin_news_down")){
			try {
				admin_news_down(request,response);
			} catch (SQLException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public void news_branch_list(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		//User user = (User)request.getSession().getAttribute("sys_user");
		DBConn db = new DBConn();
		Collection coll = new ArrayList();
		List  result = new ArrayList();
		...
		try {
			...
			db.close();
			if(message.equals("")){
			RequestDispatcher dispatcher = request.getRequestDispatcher("news_branch_list.jsp");
			dispatcher.forward(request, response);
			}else{
				RequestDispatcher dispatcher = request.getRequestDispatcher("news_message.jsp");
				dispatcher.forward(request, response);
	...
	public void news_branch_detail(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		DBConn db = new DBConn();
		String message ="";
		String id = StringUtil.notNull(request.getParameter("id"));
		try {
			...
			db.close();
			if(message.equals("")){
			RequestDispatcher dispatcher = request.getRequestDispatcher("news_branch_detail.jsp");
			dispatcher.forward(request, response);
			}else{
				RequestDispatcher dispatcher = request.getRequestDispatcher("news_message.jsp");
				dispatcher.forward(request, response);
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 String sql = "insert into news(title,sort,contents,source,publisher,state,entryTime,endTime) values('"+
						title+"','"+sort+"','"+contents+"','"+source+"','"+admin.getAdminName()+"','1','"+date+"','"+date+"')";
			...
			if(db.createConn()){
				 ...
				 conn.setAutoCommit(false);
					 ...
					 java.sql.Timestamp date = new java.sql.Timestamp(date1.getTime());
						 ...
						 String sql = "update news set title='"+title+"',source='"+source+"',contents='"+contents+"',sort='"+sort+"',endTime='"+date+"' where id='"+id+"'";
						 stmt= conn.createStatement();
						 stmt.executeUpdate(sql);
	...
	public void admin_news_down(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException, SQLException{
		request.setCharacterEncoding("UTF-8");
		response.setContentType("text/html;charset=UTF-8");
		Admin admin = (Admin)request.getSession().getAttribute("sys_admin");
		DBConn db = new DBConn();
	...
	public void admin_news_del(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException, SQLException{
		...
		try {
			...
			if(db.createConn()){
				...
				stmt= conn.createStatement();
				stmt.executeUpdate(sql);
				request.setAttribute("message", "新闻删除成功，请在新闻列表中查看新闻信息！");
				 conn.commit();
				 conn.setAutoCommit(autoCommit);
			}else {
				request.setAttribute("message", "数据库连接已断开！");
